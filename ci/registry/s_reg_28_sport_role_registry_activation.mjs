/**
 * DEV NOTE: S-REG-28 registry activation - sport_role_registry.
 * Purpose: the fourth explicit activation slice authorised after S-REG-23's
 ...
 * first).
 * Activates exactly one candidate domain (sport_role) into the active
 * registry surface, satisfying S-REG-24's contract for that target only.
 * Boundary: activates the `sport_role` active registry domain only. Must
 ...
 * create UI behaviour. Nothing consumes the sport_role registry, so
 ...
 * Failure: throws CI_S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION.
 */

import fs from "node:fs";

export const S_REG_28_SLICE_ID = "S-REG-28";
export const S_REG_28_FAILURE_TOKEN = "CI_S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION";
export const S_REG_28_ACTIVATION_ID = "sport_role_registry_activation";
export const S_REG_28_RUNTIME_STATUS = "non_runtime";
export const S_REG_28_SOURCE_HOLD_SLICE_ID = "S-REG-23";
export const S_REG_28_SOURCE_CONTRACT_SLICE_ID = "S-REG-24";
export const S_REG_28_ACTIVATION_TARGET = "sport_role_registry";
export const S_REG_28_ACTIVATED_REGISTRY_ID = "sport_role";
export const S_REG_28_SOURCE_CANDIDATE_SLICE_ID = "S-REG-10";

export const S_REG_28_REQUIRED_TRUE_FLAGS = Object.freeze([
...
export const S_REG_28_REQUIRED_FALSE_FLAGS = Object.freeze([
...
export const S_REG_28_COVERED_REQUIRED_BEFORE_ACTIVATION = Object.freeze([
...
export const S_REG_28_ACTIVE_REGISTRY_ORDER_AFTER = Object.freeze([
  "activity",
  "movement",
  "exercise",
  "program",
...
export const S_REG_28_EXPECTED_DOCUMENT_KEYS = Object.freeze([
  "slice_id",
  "activation_id",
  "decision_type",
...
export const S_REG_28_PATHS = Object.freeze({
  activation: "ci/registry/s_reg_28_sport_role_registry_activation.json",
  ...
  sport_role_registry: "registries/sport_role/sport_role.registry.json"
});

export const S_REG_28_FORBIDDEN_KEYS = Object.freeze([
  "engine_output",
...
function fail(reason, details = {}) {
  ...
  error.code = S_REG_28_FAILURE_TOKEN;
  error.reason = reason;
...
function assertNoForbiddenKeys(value, path = "root") {
  ...
  for (const key of Object.keys(value)) {
    if (S_REG_28_FORBIDDEN_KEYS.includes(key)) {
      fail("s_reg_28_forbidden_key_present", { key, path });
    }
...
function assertSourceHoldRecordsThisSupersession() {
  const hold = readJson(S_REG_28_PATHS.source_hold);
  assertPlainObject(hold, "s_reg_28_source_hold_invalid");

  if (hold.slice_id !== S_REG_28_SOURCE_HOLD_SLICE_ID || hold.decision_type !== "hold") {
    fail("s_reg_28_source_hold_identity_invalid", { slice_id: hold.slice_id, decision_type: hold.decision_type });
  }

  if (!Array.isArray(hold.superseded_by_slice_ids) || !hold.superseded_by_slice_ids.includes(S_REG_28_SLICE_ID)) {
    fail("s_reg_28_source_hold_missing_supersession_record", { actual: hold.superseded_by_slice_ids });
  }
}

function assertSourceContractRecordsThisSupersession() {
  const contract = readJson(S_REG_28_PATHS.source_contract);
  assertPlainObject(contract, "s_reg_28_source_contract_invalid");

  if (contract.slice_id !== S_REG_28_SOURCE_CONTRACT_SLICE_ID || contract.contract_type !== "design_only") {
    fail("s_reg_28_source_contract_identity_invalid", {
      slice_id: contract.slice_id,
      contract_type: contract.contract_type
    });
  }

  if (!Array.isArray(contract.superseded_by_slice_ids) || !contract.superseded_by_slice_ids.includes(S_REG_28_SLICE_ID)) {
    fail("s_reg_28_source_contract_missing_supersession_record", { actual: contract.superseded_by_slice_ids });
  }
}

function assertActiveRegistrySurfaceExtendedCorrectly() {
  const registryIndex = readJson(S_REG_28_PATHS.registry_index);
  const registryBundle = readJson(S_REG_28_PATHS.registry_bundle);
  const sportRoleRegistry = readJson(S_REG_28_PATHS.sport_role_registry);

  assertArrayStartsWith(
    registryIndex.order,
    S_REG_28_ACTIVE_REGISTRY_ORDER_AFTER,
    "s_reg_28_active_registry_index_order_invalid"
  );

  assertArrayStartsWith(
    Object.keys(registryBundle.registries),
    S_REG_28_ACTIVE_REGISTRY_ORDER_AFTER,
    "s_reg_28_active_registry_bundle_keys_invalid"
  );

  if (sportRoleRegistry.registry_id !== S_REG_28_ACTIVATED_REGISTRY_ID) {
    fail("s_reg_28_sport_role_registry_id_invalid", { actual: sportRoleRegistry.registry_id });
  }

  if (!sportRoleRegistry.entries || typeof sportRoleRegistry.entries !== "object") {
...
export function sReg28LoadSportRoleRegistryActivation() {
  return readJson(S_REG_28_PATHS.activation);
}

export function sReg28ValidateSportRoleRegistryActivation({
  activationDocument = sReg28LoadSportRoleRegistryActivation()
} = {}) {
  assertPlainObject(activationDocument, "s_reg_28_activation_document_invalid");
  assertNoForbiddenKeys(activationDocument);
  assertExactArray(
    Object.keys(activationDocument).sort(),
    [...S_REG_28_EXPECTED_DOCUMENT_KEYS].sort(),
    "s_reg_28_activation_document_keys_invalid"
  );
  ...
  if (activationDocument.slice_id !== S_REG_28_SLICE_ID) {
    fail("s_reg_28_slice_id_invalid", { actual: activationDocument.slice_id });
  }

  if (activationDocument.activation_id !== S_REG_28_ACTIVATION_ID || activationDocument.decision_type !== "activation") {
    fail("s_reg_28_activation_identity_invalid", {
  ...
  if (
    activationDocument.source_hold_slice_id !== S_REG_28_SOURCE_HOLD_SLICE_ID ||
    activationDocument.source_contract_slice_id !== S_REG_28_SOURCE_CONTRACT_SLICE_ID ||
    activationDocument.source_candidate_slice_id !== S_REG_28_SOURCE_CANDIDATE_SLICE_ID
  ) {
    fail("s_reg_28_source_reference_invalid", {
  ...
  if (activationDocument.runtime_status !== S_REG_28_RUNTIME_STATUS) {
    fail("s_reg_28_runtime_status_invalid", { actual: activationDocument.runtime_status });
  }

  if (activationDocument.activation_decision !== "authorised") {
    fail("s_reg_28_activation_decision_invalid", { actual: activationDocument.activation_decision });
  }

  if (
    activationDocument.activation_target !== S_REG_28_ACTIVATION_TARGET ||
    activationDocument.activated_registry_id !== S_REG_28_ACTIVATED_REGISTRY_ID
  ) {
    fail("s_reg_28_activation_target_invalid", {
  ...
  for (const flag of S_REG_28_REQUIRED_TRUE_FLAGS) {
    if (activationDocument[flag] !== true) {
      fail("s_reg_28_true_flag_invalid", { flag, actual: activationDocument[flag] });
    }
  }

  for (const flag of S_REG_28_REQUIRED_FALSE_FLAGS) {
    if (activationDocument[flag] !== false) {
      fail("s_reg_28_false_flag_invalid", { flag, actual: activationDocument[flag] });
    }
  }
  ...
  assertExactArray(
    ...
    "s_reg_28_covered_required_before_activation_invalid"
  );

  assertExactArray(
    activationDocument.active_registry_order_after,
    S_REG_28_ACTIVE_REGISTRY_ORDER_AFTER,
    "s_reg_28_active_registry_order_after_invalid"
  );

  if (activationDocument.activated_record_count !== activatedRecordCount) {
    fail("s_reg_28_activated_record_count_invalid", {
      declared: activationDocument.activated_record_count,
      actual: activatedRecordCount
    });
  }

  assertPlainObject(activationDocument.human_authorisation, "s_reg_28_human_authorisation_invalid");
  if (
    ...
    fail("s_reg_28_human_authorisation_invalid", { actual: activationDocument.human_authorisation });
  }

  assertPlainObject(activationDocument.active_registry_hashes_before, "s_reg_28_hashes_before_invalid");
  assertPlainObject(activationDocument.active_registry_hashes_after, "s_reg_28_hashes_after_invalid");
  for (const key of ["registry_index", "registry_bundle"]) {
    if (
      ...
      fail("s_reg_28_hashes_invalid", { key });
    }
  }

  assertPlainObject(activationDocument.rollback_plan, "s_reg_28_rollback_plan_invalid");
  if (
    ...
    fail("s_reg_28_rollback_plan_invalid", { actual: activationDocument.rollback_plan });
  }

  assertPlainObject(activationDocument.runtime_parity_proof, "s_reg_28_runtime_parity_proof_invalid");
  ...
  if (
    ...
    fail("s_reg_28_runtime_parity_proof_invalid", { actual: parity });
  }
  ...
  return Object.freeze({
    ok: true,
    token: S_REG_28_FAILURE_TOKEN,
    slice_id: S_REG_28_SLICE_ID,
    activation_id: activationDocument.activation_id,
    decision_type: activationDocument.decision_type,
    ...
    runtime_status: S_REG_28_RUNTIME_STATUS,
