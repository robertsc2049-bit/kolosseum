...
// @scope: registry
/**
 * DEV NOTE: S-REG-31 guard.
 * Purpose: proves the exercise_token_registry activation is genuinely
 * authorised, scoped to exactly the exercise_token domain, satisfies every
 * proof command S-REG-24's contract requires, and that every activated
 * record's cross-registry references (movement_id, activity_ids) are real -
 * not a repeat of the source candidate's dangling front_plank_token
 ...
 * allows for this target (registry index/bundle, the new exercise_token
 * registry file and its schema, the surface classification, the four seal
 * evidence files, and this slice's own scaffolding). This branch was created
 * off the already-merged origin/main (which contains S-REG-25 through
 * S-REG-30), so its diff is clean and scoped to this slice only. It must not
 * permit any change under engine/, src/, server/, app/, web/,
 * shared/pilot-lifecycle/, or shared/v1-boundary/, nor any other candidate
 * domain's activation.
 * Failure: emits CI_S_REG_31_EXERCISE_TOKEN_REGISTRY_ACTIVATION.
 */
...
import {
  ...
  S_REG_31_RUNTIME_STATUS,
  sReg31LoadExerciseTokenRegistryActivation,
  sReg31ValidateExerciseTokenRegistryActivation
} from "../registry/s_reg_31_exercise_token_registry_activation.mjs";

const repoRoot = process.cwd();
const GUARD = "S-REG-31";
const TOKEN = S_REG_31_FAILURE_TOKEN;

const allowedChangedFiles = new Set([
  "ci/registry/s_reg_31_exercise_token_registry_activation.mjs",
  "ci/registry/s_reg_31_exercise_token_registry_activation.json",
  "test/s_reg_31_exercise_token_registry_activation.test.mjs",
  "ci/guards/s_reg_31_exercise_token_registry_activation_guard.mjs",
  "docs/roadmap/S_REG_31_EXERCISE_TOKEN_REGISTRY_ACTIVATION.md",
  "registries/exercise_token/",
  "registries/exercise_token/exercise_token.registry.json",
  "ci/schemas/exercise_token.registry.schema.json",
  ...
  // "exercise_token", so exactly these two golden fixtures (the only ones
  ...
  // S-REG-25/26/27/28/29/30's own test files hardcoded a permanent
  // "exercise_token was not activated" existsSync(false) assertion, which
  // broke the moment this slice activated exercise_token - removed the
  // stale check, the same class of fix already applied repeatedly.
...
// this slice activates exercise_token only.
...
function assertIncludes(text, marker, context) {
  if (!text.includes(marker)) {
    fail("Required S-REG-31 marker is missing.", { marker, context });
  }
}
...
function collectChangedFiles() {
  ...
  if (!branchName.includes("s-reg-31-registry-activation-exercise-token")) {
    return [];
  }
...
function assertChangedFilesWithinBoundary() {
  ...
  if (disallowed.length > 0) {
    fail("S-REG-31 touched files outside the exercise_token_registry activation boundary.", {
  ...
  for (const relativePath of changed) {
    if (forbiddenChangedFiles.has(relativePath)) {
      fail("S-REG-31 touched another candidate domain's active registry file - this slice activates exercise_token only.", { path: relativePath });
    }

    for (const prefix of forbiddenChangedPrefixes) {
      if (relativePath.startsWith(prefix)) {
        fail("S-REG-31 touched a forbidden engine/runtime surface.", {
          path: relativePath,
...
function assertRequiredFilesPresent() {
  for (const relativePath of [
    ...
    if (!fs.existsSync(repoPath(relativePath))) {
      fail("Required S-REG-31 file is missing.", { path: relativePath });
    }
  }
}
...
function assertPackageWiring() {
  ...
  const expected = "node --test test/s_reg_31_exercise_token_registry_activation.test.mjs && node ci/guards/s_reg_31_exercise_token_registry_activation_guard.mjs";

  if (packageJson.scripts?.["proof:s-reg-31"] !== expected) {
    fail("S-REG-31 package proof script is missing or incorrect.", {
      actual: packageJson.scripts?.["proof:s-reg-31"],
      expected
    });
  }

  if (packageJson.scripts?.["lint:fast"]?.includes("s_reg_31_exercise_token_registry_activation")) {
    fail("S-REG-31 must not append to the top-level lint:fast command.");
  }

  if (!packageJson.scripts?.["lint:fast:inline"]?.includes("s_reg_31_exercise_token_registry_activation")) {
    fail("S-REG-31 lint:fast:inline package wiring is missing.");
  }

  if (!packageJson.scripts?.["test:unit"]?.includes("s_reg_31_exercise_token_registry_activation_guard.mjs")) {
    fail("S-REG-31 test:unit guard wiring is missing.");
  }
}
...
function assertGeneratedIndexes() {
  ...
  assertIncludes(
    failureTokenIndex,
    "CI_S_REG_31_EXERCISE_TOKEN_REGISTRY_ACTIVATION",
    "docs/dev/FAILURE_TOKEN_INDEX.md"
  );
}

function assertDocMarkers() {
  const doc = readText("docs/roadmap/S_REG_31_EXERCISE_TOKEN_REGISTRY_ACTIVATION.md");

  for (const marker of [
    "S-REG-31",
    "exercise_token_registry_activation",
    "exercise_token",
    "front_plank_token",
    "human-authorised",
    "rollback",
    ...
    assertIncludes(doc, marker, "docs/roadmap/S_REG_31_EXERCISE_TOKEN_REGISTRY_ACTIVATION.md");
  }
}
...
function assertRequiredProofCommandsPass() {
  for (const [command, ...args] of REQUIRED_PROOF_COMMANDS) {
    ...
    if (typeof result !== "string") {
      fail("S-REG-31 required proof command did not run.", { command, args });
    }
  }
}
...
function main() {
  ...
  if (
    result.ok !== true ||
    result.activation_id !== "exercise_token_registry_activation" ||
    ...
    result.runtime_status !== S_REG_31_RUNTIME_STATUS
  ) {
    fail("S-REG-31 validation result is invalid.", { result });
  }
