...
// @scope: registry
/**
 * DEV NOTE: S-REG-29 guard.
 * Purpose: proves the metric_exercise_link_registry activation is genuinely
 * authorised, scoped to exactly the metric_exercise_link domain, satisfies
 * every proof command S-REG-24's contract requires, and that every activated
 * record's cross-registry references (exercise_id, sport_metric_id) are
 * real - not a repeat of the source candidate's dangling front_plank
 * reference, which was explicitly excluded rather than shipped.
 * Boundary: permits only the exact mutation surface S-REG-24's contract
 * allows for this target (registry index/bundle, the new
 * metric_exercise_link registry file and its schema, the surface
 * classification, the four seal evidence files, and this slice's own
 * scaffolding). This branch was created off the already-merged origin/main
 * (which contains S-REG-25/26/27/28), so its diff is clean and scoped to
 * this slice only, plus the prior activation test/doc files this slice must
 * fix (see below). It must not permit any change under engine/, src/,
 * server/, app/, web/, shared/pilot-lifecycle/, or shared/v1-boundary/, nor
 * any other candidate domain's activation.
 * Failure: emits CI_S_REG_29_METRIC_EXERCISE_LINK_REGISTRY_ACTIVATION.
 */
...
import {
  ...
  S_REG_29_RUNTIME_STATUS,
  sReg29LoadMetricExerciseLinkRegistryActivation,
  sReg29ValidateMetricExerciseLinkRegistryActivation
} from "../registry/s_reg_29_metric_exercise_link_registry_activation.mjs";

const repoRoot = process.cwd();
const GUARD = "S-REG-29";
const TOKEN = S_REG_29_FAILURE_TOKEN;

const allowedChangedFiles = new Set([
  "ci/registry/s_reg_29_metric_exercise_link_registry_activation.mjs",
  "ci/registry/s_reg_29_metric_exercise_link_registry_activation.json",
  "test/s_reg_29_metric_exercise_link_registry_activation.test.mjs",
  "ci/guards/s_reg_29_metric_exercise_link_registry_activation_guard.mjs",
  "docs/roadmap/S_REG_29_METRIC_EXERCISE_LINK_REGISTRY_ACTIVATION.md",
  "registries/metric_exercise_link/",
  "registries/metric_exercise_link/metric_exercise_link.registry.json",
  "ci/schemas/metric_exercise_link.registry.schema.json",
  ...
  // "metric_exercise_link", so exactly these two golden fixtures (the only
  // ones that exercise phase3's constraints-resolution output) needed
  // re-pinning.
  ...
  // S-REG-25/26/27/28's own test files hardcoded a permanent
  // "metric_exercise_link was not activated" existsSync(false) assertion,
  // which broke the moment this slice activated metric_exercise_link -
  // removed the stale check, the same class of fix already applied
  // repeatedly for sport_metric (S-REG-27) and sport_role (S-REG-28).
...
// this slice activates metric_exercise_link only.
...
function assertIncludes(text, marker, context) {
  if (!text.includes(marker)) {
    fail("Required S-REG-29 marker is missing.", { marker, context });
  }
}
...
function collectChangedFiles() {
  ...
  if (!branchName.includes("s-reg-29-registry-activation-metric-exercise-link")) {
    return [];
  }
...
function assertChangedFilesWithinBoundary() {
  ...
  if (disallowed.length > 0) {
    fail("S-REG-29 touched files outside the metric_exercise_link_registry activation boundary.", {
  ...
  for (const relativePath of changed) {
    if (forbiddenChangedFiles.has(relativePath)) {
      fail("S-REG-29 touched another candidate domain's active registry file - this slice activates metric_exercise_link only.", { path: relativePath });
    }

    for (const prefix of forbiddenChangedPrefixes) {
      if (relativePath.startsWith(prefix)) {
        fail("S-REG-29 touched a forbidden engine/runtime surface.", {
          path: relativePath,
...
function assertRequiredFilesPresent() {
  for (const relativePath of [
    ...
    if (!fs.existsSync(repoPath(relativePath))) {
      fail("Required S-REG-29 file is missing.", { path: relativePath });
    }
  }
}
...
function assertPackageWiring() {
  ...
  const expected = "node --test test/s_reg_29_metric_exercise_link_registry_activation.test.mjs && node ci/guards/s_reg_29_metric_exercise_link_registry_activation_guard.mjs";

  if (packageJson.scripts?.["proof:s-reg-29"] !== expected) {
    fail("S-REG-29 package proof script is missing or incorrect.", {
      actual: packageJson.scripts?.["proof:s-reg-29"],
      expected
    });
  }

  if (packageJson.scripts?.["lint:fast"]?.includes("s_reg_29_metric_exercise_link_registry_activation")) {
    fail("S-REG-29 must not append to the top-level lint:fast command.");
  }

  if (!packageJson.scripts?.["lint:fast:inline"]?.includes("s_reg_29_metric_exercise_link_registry_activation")) {
    fail("S-REG-29 lint:fast:inline package wiring is missing.");
  }

  if (!packageJson.scripts?.["test:unit"]?.includes("s_reg_29_metric_exercise_link_registry_activation_guard.mjs")) {
    fail("S-REG-29 test:unit guard wiring is missing.");
  }
}
...
function assertGeneratedIndexes() {
  ...
  assertIncludes(
    failureTokenIndex,
    "CI_S_REG_29_METRIC_EXERCISE_LINK_REGISTRY_ACTIVATION",
    "docs/dev/FAILURE_TOKEN_INDEX.md"
  );
}

function assertDocMarkers() {
  const doc = readText("docs/roadmap/S_REG_29_METRIC_EXERCISE_LINK_REGISTRY_ACTIVATION.md");

  for (const marker of [
    "S-REG-29",
    "metric_exercise_link_registry_activation",
    ...
    assertIncludes(doc, marker, "docs/roadmap/S_REG_29_METRIC_EXERCISE_LINK_REGISTRY_ACTIVATION.md");
  }
}
...
function assertRequiredProofCommandsPass() {
  for (const [command, ...args] of REQUIRED_PROOF_COMMANDS) {
    ...
    if (typeof result !== "string") {
      fail("S-REG-29 required proof command did not run.", { command, args });
    }
  }
}
...
function main() {
  ...
  if (
    result.ok !== true ||
    result.activation_id !== "metric_exercise_link_registry_activation" ||
    ...
    result.runtime_status !== S_REG_29_RUNTIME_STATUS
  ) {
    fail("S-REG-29 validation result is invalid.", { result });
  }
  ...
  console.log(JSON.stringify({
    ...
    excluded_candidate_record_ids: result.excluded_candidate_record_ids,
