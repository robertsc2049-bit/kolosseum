...
// @scope: registry
/**
 * DEV NOTE: S-REG-28 guard.
 * Purpose: proves the sport_role_registry activation is genuinely
 * authorised, scoped to exactly the sport_role domain, and satisfies every
 * proof command S-REG-24's contract requires.
 * Boundary: permits only the exact mutation surface S-REG-24's contract
 * allows for this target (registry index/bundle, the new sport_role
 * registry file and its schema, the surface classification, the four seal
 * evidence files, and this slice's own scaffolding). This branch was
 * created off the already-merged origin/main (which contains S-REG-25/26/27),
 * so its diff is clean and scoped to this slice only, plus the S-REG-25/26/27
 * test/doc files this slice must fix (see below). It must not permit any
 * change under engine/, src/, server/, app/, web/, shared/pilot-lifecycle/,
 * or shared/v1-boundary/, nor any other candidate domain's activation.
 * Failure: emits CI_S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION.
 */
...
import {
  ...
  S_REG_28_RUNTIME_STATUS,
  sReg28LoadSportRoleRegistryActivation,
  sReg28ValidateSportRoleRegistryActivation
} from "../registry/s_reg_28_sport_role_registry_activation.mjs";

const repoRoot = process.cwd();
const GUARD = "S-REG-28";
const TOKEN = S_REG_28_FAILURE_TOKEN;

const allowedChangedFiles = new Set([
  "ci/registry/s_reg_28_sport_role_registry_activation.mjs",
  "ci/registry/s_reg_28_sport_role_registry_activation.json",
  "test/s_reg_28_sport_role_registry_activation.test.mjs",
  "ci/guards/s_reg_28_sport_role_registry_activation_guard.mjs",
  "docs/roadmap/S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION.md",
  "registries/sport_role/",
  "registries/sport_role/sport_role.registry.json",
  "ci/schemas/sport_role.registry.schema.json",
  ...
  // which registry files were loaded - it truthfully gained "sport_role", so
  // exactly these two golden fixtures (the only ones that exercise phase3's
  // constraints-resolution output) needed re-pinning.
  ...
  // S-REG-25/26/27's own test files hardcoded a permanent "sport_role was
  // not activated" existsSync(false) assertion, which broke the moment this
  // slice activated sport_role - removed the stale check, the same class of
  // fix already applied for sport_metric during S-REG-27.
...
// this slice activates sport_role only.
...
function assertIncludes(text, marker, context) {
  if (!text.includes(marker)) {
    fail("Required S-REG-28 marker is missing.", { marker, context });
  }
}
...
function collectChangedFiles() {
  ...
  if (!branchName.includes("s-reg-28-registry-activation-sport-role")) {
    return [];
  }
...
function assertChangedFilesWithinBoundary() {
  ...
  if (disallowed.length > 0) {
    fail("S-REG-28 touched files outside the sport_role_registry activation boundary.", {
  ...
  for (const relativePath of changed) {
    if (forbiddenChangedFiles.has(relativePath)) {
      fail("S-REG-28 touched another candidate domain's active registry file - this slice activates sport_role only.", { path: relativePath });
    }

    for (const prefix of forbiddenChangedPrefixes) {
      if (relativePath.startsWith(prefix)) {
        fail("S-REG-28 touched a forbidden engine/runtime surface.", {
          path: relativePath,
...
function assertRequiredFilesPresent() {
  for (const relativePath of [
    ...
    if (!fs.existsSync(repoPath(relativePath))) {
      fail("Required S-REG-28 file is missing.", { path: relativePath });
    }
  }
}
...
function assertPackageWiring() {
  ...
  const expected = "node --test test/s_reg_28_sport_role_registry_activation.test.mjs && node ci/guards/s_reg_28_sport_role_registry_activation_guard.mjs";

  if (packageJson.scripts?.["proof:s-reg-28"] !== expected) {
    fail("S-REG-28 package proof script is missing or incorrect.", {
      actual: packageJson.scripts?.["proof:s-reg-28"],
      expected
    });
  }

  if (packageJson.scripts?.["lint:fast"]?.includes("s_reg_28_sport_role_registry_activation")) {
    fail("S-REG-28 must not append to the top-level lint:fast command.");
  }

  if (!packageJson.scripts?.["lint:fast:inline"]?.includes("s_reg_28_sport_role_registry_activation")) {
    fail("S-REG-28 lint:fast:inline package wiring is missing.");
  }

  if (!packageJson.scripts?.["test:unit"]?.includes("s_reg_28_sport_role_registry_activation_guard.mjs")) {
    fail("S-REG-28 test:unit guard wiring is missing.");
  }
}
...
function assertGeneratedIndexes() {
  ...
  assertIncludes(
    failureTokenIndex,
    "CI_S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION",
    "docs/dev/FAILURE_TOKEN_INDEX.md"
  );
}

function assertDocMarkers() {
  const doc = readText("docs/roadmap/S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION.md");

  for (const marker of [
    "S-REG-28",
    "sport_role_registry_activation",
    "sport_role",
    "human-authorised",
    "rollback",
    "runtime parity",
    ...
    assertIncludes(doc, marker, "docs/roadmap/S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION.md");
  }
}
...
function assertRequiredProofCommandsPass() {
  for (const [command, ...args] of REQUIRED_PROOF_COMMANDS) {
    ...
    if (typeof result !== "string") {
      fail("S-REG-28 required proof command did not run.", { command, args });
    }
  }
}
...
function main() {
  ...
  if (
    result.ok !== true ||
    result.activation_id !== "sport_role_registry_activation" ||
    ...
    result.runtime_status !== S_REG_28_RUNTIME_STATUS
  ) {
    fail("S-REG-28 validation result is invalid.", { result });
  }
