...
// @scope: registry
/**
 * DEV NOTE: S-REG-24 guard.
 * Purpose: proves the activation path after S-REG-23 is contract design only.
 * Boundary: permits only S-REG-24 contract-design files, package wiring,
 ...
 * Failure: emits CI_S_REG_24_REGISTRY_ACTIVATION_CONTRACT_DESIGN.
 */
...
import {
  ...
  S_REG_24_RUNTIME_STATUS,
  sReg24LoadRegistryActivationContractDesign,
  sReg24ValidateRegistryActivationContractDesign
} from "../registry/s_reg_24_registry_activation_contract_design.mjs";

const repoRoot = process.cwd();
const GUARD = "S-REG-24";
const TOKEN = S_REG_24_FAILURE_TOKEN;
...
const allowedChangedFiles = new Set([
  ...
  "ci/guards/s_reg_24_registry_activation_contract_design_guard.mjs",
...
function assertIncludes(text, marker, context) {
  if (!text.includes(marker)) {
    fail("Required S-REG-24 marker is missing.", { marker, context });
  }
}
...
function collectChangedFiles() {
  ...
  if (!branchName.includes("s-reg-24-registry-activation-contract-design")) {
    return [];
  }
...
function assertChangedFilesWithinBoundary() {
  ...
  if (disallowed.length > 0) {
    fail("S-REG-24 touched files outside the registry activation contract design boundary.", {
  ...
  for (const relativePath of changed) {
    if (forbiddenChangedFiles.has(relativePath)) {
      fail("S-REG-24 touched a forbidden active, seal, or candidate dependency surface.", { path: relativePath });
    }

    for (const prefix of forbiddenChangedPrefixes) {
      if (relativePath.startsWith(prefix) && !allowedChangedFiles.has(relativePath)) {
        fail("S-REG-24 touched a forbidden active or runtime surface.", {
          path: relativePath,
...
function assertRequiredFilesPresent() {
  for (const relativePath of [
    ...
    if (!fs.existsSync(repoPath(relativePath))) {
      fail("Required S-REG-24 file is missing.", { path: relativePath });
    }
  }
}
...
function assertPackageWiring() {
  ...
  const expected = "node --test test/s_reg_24_registry_activation_contract_design.test.mjs && node ci/guards/s_reg_24_registry_activation_contract_design_guard.mjs";

  if (packageJson.scripts?.["proof:s-reg-24"] !== expected) {
    fail("S-REG-24 package proof script is missing or incorrect.", {
      actual: packageJson.scripts?.["proof:s-reg-24"],
      expected
    });
  }

  if (packageJson.scripts?.["lint:fast"]?.includes("s_reg_24_registry_activation_contract_design")) {
    fail("S-REG-24 must not append to the top-level lint:fast command.");
  }

  if (!packageJson.scripts?.["lint:fast:inline"]?.includes("s_reg_24_registry_activation_contract_design")) {
    fail("S-REG-24 lint:fast:inline package wiring is missing.");
  }

  if (!packageJson.scripts?.["test:unit"]?.includes("s_reg_24_registry_activation_contract_design_guard.mjs")) {
    fail("S-REG-24 test:unit guard wiring is missing.");
  }
}
...
function assertGeneratedIndexes() {
  ...
  assertIncludes(
    failureTokenIndex,
    "CI_S_REG_24_REGISTRY_ACTIVATION_CONTRACT_DESIGN",
    "docs/dev/FAILURE_TOKEN_INDEX.md"
  );
}

function assertDocMarkers() {
  const doc = readText("docs/roadmap/S_REG_24_REGISTRY_ACTIVATION_CONTRACT_DESIGN.md");

  for (const marker of [
    "S-REG-24",
    "registry_activation_contract_design",
    ...
    assertIncludes(doc, marker, "docs/roadmap/S_REG_24_REGISTRY_ACTIVATION_CONTRACT_DESIGN.md");
  }
}

function assertSourceHoldStillHeld() {
  const hold = readJson("ci/registry/s_reg_23_registry_activation_hold_decision.json");

  if (hold.activation_decision !== "hold") {
    fail("S-REG-23 source hold decision drifted.", { activation_decision: hold.activation_decision });
  }

  if (
    hold.activation_authorised !== false ||
    hold.activation_ready !== false ||
    hold.active_registry_activation !== false
  ) {
    fail("S-REG-23 source hold activation flags drifted.", {
...
function main() {
  ...
  if (
    result.ok !== true ||
    result.contract_id !== "registry_activation_contract_design" ||
    result.contract_type !== "design_only" ||
    result.runtime_status !== S_REG_24_RUNTIME_STATUS ||
    result.activation_authorised !== false ||
    result.activation_ready !== false ||
    result.active_registry_activation !== false
  ) {
    fail("S-REG-24 validation result is invalid.", { result });
  }
  ...
  console.log(JSON.stringify({
    ...
    contract_design_status: result.contract_design_status,
    source_hold_slice_id: result.source_hold_slice_id,
    activation_authorised: false,
    activation_ready: false,
    active_registry_activation: false,
    runtime_status: S_REG_24_RUNTIME_STATUS,
    covered_requirement_count: result.covered_requirement_count,
    future_allowed_activation_mutation_count: result.future_allowed_activation_mutation_count,
    future_forbidden_activation_shortcut_count: result.future_forbidden_activation_shortcut_count,
    future_registry_load_order_edge_count: result.future_registry_load_order_edge_count,
    future_required_proof_command_count: result.future_required_proof_command_count,
