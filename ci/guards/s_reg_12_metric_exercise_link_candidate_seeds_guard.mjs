...
/**
 * DEV NOTE: S-REG-12 guard.
 * Purpose: proves inert metric-exercise link candidate seeds exist, FK-close
 * against sport metric and exercise candidates, and remain outside active
 * registry law.
 * Boundary: permits only S-REG-12 metric-exercise link candidate files, proof
 * files, documentation, package wiring, and generated indexes/checksums. It
 * must not permit active registry, registry law, bundle writer, engine runtime,
 * threshold marker, marker evaluator, programme template, substitution,
 * marketplace, organisation, unit, federation, team, or tactical runtime scope.
 ...
 * Failure: emits CI_S_REG_12_METRIC_EXERCISE_LINK_CANDIDATE_SEEDS.
 */
...
const GUARD = "S-REG-12";
const TOKEN = "CI_S_REG_12_METRIC_EXERCISE_LINK_CANDIDATE_SEEDS";
...
const files = Object.freeze({
  module: "ci/registry/s_reg_12_metric_exercise_link_candidate_seeds.mjs",
  manifest: "ci/registry/s_reg_12_metric_exercise_link_candidate_manifest.json",
  test: "test/s_reg_12_metric_exercise_link_candidate_seeds.test.mjs",
  guard: "ci/guards/s_reg_12_metric_exercise_link_candidate_seeds_guard.mjs",
  doc: "docs/roadmap/S_REG_12_METRIC_EXERCISE_LINK_CANDIDATE_SEEDS.md",
  linkCandidate: "ci/registry/candidates/metric_exercise_link_registry_1c_a/metric_exercise_link_registry_1c_a.candidate.registry.json",
...
function readText(relativePath) {
  ...
  if (!fs.existsSync(fullPath)) {
    fail("Required S-REG-12 file is missing.", { path: relativePath });
  }
...
function readJson(relativePath) {
  try {
    ...
    fail("Required S-REG-12 JSON file is invalid.", {
      path: relativePath,
...
function assertIncludes(text, marker, context) {
  if (!text.includes(marker)) {
    fail("Required S-REG-12 marker is missing.", {
      context,
      marker
    });
  }
...
function assertChangedFilesAllowed() {
  ...
  if (!branchName.includes("s-reg-12-metric-exercise-link-candidate-seeds")) {
    return;
  }
  ...
  if (disallowed.length > 0) {
    fail("S-REG-12 touched files outside the metric-exercise link candidate seed boundary.", {
  ...
  for (const relativePath of changed) {
    if (forbiddenChangedFiles.has(relativePath)) {
      fail("S-REG-12 touched a threshold-marker candidate surface.", { path: relativePath });
    }

    for (const prefix of forbiddenChangedPrefixes) {
      if (relativePath.startsWith(prefix)) {
        fail("S-REG-12 touched a forbidden active or runtime surface.", {
          path: relativePath,
...
function assertActiveRegistryStillCompact() {
  ...
  for (const registryId of [
    ...
    if (fs.existsSync(repoPath(`registries/${registryId}`))) {
      fail("S-REG-12 must not create active canonical metric-exercise link, threshold-marker, sport metric, or exercise registry directories.", {
...
function assertCandidateBoundary() {
  const document = readJson(files.linkCandidate);
  ...
  for (const [field, expected] of Object.entries({
    ...
    metric_exercise_link_seed_status: "candidate_fk_ready"
  })) {
    if (document[field] !== expected) {
      fail("S-REG-12 candidate boundary field mismatch.", {
        field,
        expected,
  ...
  if (document.records.length !== 13) {
    fail("S-REG-12 candidate record count must remain fixed.", {
      metric_exercise_link_count: document.records.length
    });
  }

  if (document.records.some((record) => record.sport_metric_id === "rugby_union__sprint_time_seconds")) {
    fail("S-REG-12 must not link rugby sprint time until a factual sprint exercise candidate exists.");
  }
}

function assertNoForbiddenCandidateText() {
  const combined = readText(files.linkCandidate).toLowerCase();

  for (const term of forbiddenCandidateTextTerms) {
    if (combined.includes(term.toLowerCase())) {
      fail("S-REG-12 candidate data contains forbidden threshold, marker, evaluator, claim, or decision text.", { term });
    }
  }
}
...
async function main() {
  ...
  const result = module.sReg12ValidateMetricExerciseLinkCandidateSeeds();

  if (!result.ok || result.metric_exercise_link_count !== 13 || result.sport_metric_count !== 6 || result.exercise_count !== 4 || result.activity_count !== 3) {
    fail("S-REG-12 module validation failed.", { result });
  }

  if (result.activation_ready !== false || result.runtime_status !== "non_runtime") {
    fail("S-REG-12 result must remain non-runtime and not activation-ready.", { result });
  }
  ...
  assertDeepEqual(
    ...
    ["metric_exercise_link_registry_1c_a"],
    "S-REG-12 manifest candidate path order changed."
  );
  ...
  assertIncludes(guardsIndexText, "s_reg_12_metric_exercise_link_candidate_seeds_guard", "docs/GUARDS_INDEX.md");
  ...
  console.log(JSON.stringify({
    ...
    guard: GUARD,
    token: TOKEN,
    metric_exercise_link_count: result.metric_exercise_link_count,
    ...
    message: "S-REG-12 metric-exercise link candidate seeds passed."
  }, null, 2));
}
