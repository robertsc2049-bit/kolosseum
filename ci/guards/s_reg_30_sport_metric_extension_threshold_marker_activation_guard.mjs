...
// @scope: registry
/**
 * DEV NOTE: S-REG-30 guard.
 * Purpose: proves the sport_metric extension + threshold_marker_registry
 * activation is genuinely authorised, scoped to exactly those two domains,
 * satisfies every proof command S-REG-24's contract requires, and that
 * every activated threshold_marker record's cross-registry references
 * (sport_metric_id, activity_id) are real.
 * Boundary: permits only the exact mutation surface this slice needs -
 * the extended sport_metric registry file (content only, not a new file),
 * the new threshold_marker registry file and its schema, registry
 * index/bundle/surface classification, the four seal evidence files, this
 * slice's own scaffolding, S-REG-27's fixed module/test/doc (the live-
 * count-drift fix this slice's own sport_metric extension necessitated),
 * and the preemptive existsSync sweep across S-REG-25/26/28/29's test/doc
 * files. It must not permit any change under engine/, src/, server/, app/,
 * web/, shared/pilot-lifecycle/, or shared/v1-boundary/, nor any other
 * candidate domain's activation.
 * Failure: emits CI_S_REG_30_SPORT_METRIC_EXTENSION_THRESHOLD_MARKER_ACTIVATION.
 */
...
import {
  ...
  S_REG_30_RUNTIME_STATUS,
  sReg30LoadSportMetricExtensionThresholdMarkerActivation,
  sReg30ValidateSportMetricExtensionThresholdMarkerActivation
} from "../registry/s_reg_30_sport_metric_extension_threshold_marker_activation.mjs";

const repoRoot = process.cwd();
const GUARD = "S-REG-30";
const TOKEN = S_REG_30_FAILURE_TOKEN;

const allowedChangedFiles = new Set([
  "ci/registry/s_reg_30_sport_metric_extension_threshold_marker_activation.mjs",
  "ci/registry/s_reg_30_sport_metric_extension_threshold_marker_activation.json",
  "test/s_reg_30_sport_metric_extension_threshold_marker_activation.test.mjs",
  "ci/guards/s_reg_30_sport_metric_extension_threshold_marker_activation_guard.mjs",
  "docs/roadmap/S_REG_30_SPORT_METRIC_EXTENSION_THRESHOLD_MARKER_ACTIVATION.md",
  // sport_metric is extended (content change), not newly created.
  "registries/sport_metric/sport_metric.registry.json",
  "registries/threshold_marker/",
  "registries/threshold_marker/threshold_marker.registry.json",
  "ci/schemas/threshold_marker.registry.schema.json",
  ...
  // "threshold_marker", so exactly these two golden fixtures (the only ones
  // that exercise phase3's constraints-resolution output) needed re-pinning.
  ...
  // S-REG-27's own module/test hardcoded an exact match between the live
  // sport_metric entry count and its own frozen activated_record_count -
  // this slice's own sport_metric extension (6 -> 9 entries) broke that
  // exact match, so it was relaxed to a membership check (the 6 originally-
  // activated sport_metric_ids must still be present). S-REG-27's own
  // frozen activated_record_count (6) is unchanged - a historical fact.
  ...
  // S-REG-25/26/28/29's own test files hardcoded a permanent
  // "threshold_marker was not activated" existsSync(false) assertion, which
  // broke the moment this slice activated threshold_marker - removed the
  // stale check, the same class of fix already applied repeatedly for
  // sport_metric/sport_role/metric_exercise_link. S-REG-29's own test also
  // had a latent exact-match (not prefix-match) bug in its live order/
  // bundle-key checks, the same copy-paste class already fixed for S-REG-28
  // during S-REG-29's own development - fixed here since it surfaced now.
  "test/s_reg_25_equipment_registry_activation.test.mjs",
  "test/s_reg_26_sport_subdivision_registry_activation.test.mjs",
  "test/s_reg_28_sport_role_registry_activation.test.mjs",
  "test/s_reg_29_metric_exercise_link_registry_activation.test.mjs",
  "docs/roadmap/S_REG_25_EQUIPMENT_REGISTRY_ACTIVATION.md",
  "docs/roadmap/S_REG_26_SPORT_SUBDIVISION_REGISTRY_ACTIVATION.md",
  "docs/roadmap/S_REG_28_SPORT_ROLE_REGISTRY_ACTIVATION.md",
  "docs/roadmap/S_REG_29_METRIC_EXERCISE_LINK_REGISTRY_ACTIVATION.md",
  "package.json",
...
// this slice extends sport_metric and activates threshold_marker only.
...
function assertIncludes(text, marker, context) {
  if (!text.includes(marker)) {
    fail("Required S-REG-30 marker is missing.", { marker, context });
  }
}
...
function collectChangedFiles() {
  ...
  if (!branchName.includes("s-reg-30-sport-metric-extension-threshold-marker-activation")) {
    return [];
  }
...
function assertChangedFilesWithinBoundary() {
  ...
  if (disallowed.length > 0) {
    fail("S-REG-30 touched files outside the sport_metric extension / threshold_marker activation boundary.", {
  ...
  for (const relativePath of changed) {
    if (forbiddenChangedFiles.has(relativePath)) {
      fail("S-REG-30 touched another candidate domain's active registry file - this slice extends sport_metric and activates threshold_marker only.", { path: relativePath });
    }

    for (const prefix of forbiddenChangedPrefixes) {
      if (relativePath.startsWith(prefix)) {
        fail("S-REG-30 touched a forbidden engine/runtime surface.", {
          path: relativePath,
...
function assertRequiredFilesPresent() {
  for (const relativePath of [
    ...
    if (!fs.existsSync(repoPath(relativePath))) {
      fail("Required S-REG-30 file is missing.", { path: relativePath });
    }
  }
}
...
function assertPackageWiring() {
  ...
  const expected = "node --test test/s_reg_30_sport_metric_extension_threshold_marker_activation.test.mjs && node ci/guards/s_reg_30_sport_metric_extension_threshold_marker_activation_guard.mjs";

  if (packageJson.scripts?.["proof:s-reg-30"] !== expected) {
    fail("S-REG-30 package proof script is missing or incorrect.", {
      actual: packageJson.scripts?.["proof:s-reg-30"],
      expected
    });
  }

  if (packageJson.scripts?.["lint:fast"]?.includes("s_reg_30_sport_metric_extension_threshold_marker_activation")) {
    fail("S-REG-30 must not append to the top-level lint:fast command.");
  }

  if (!packageJson.scripts?.["lint:fast:inline"]?.includes("s_reg_30_sport_metric_extension_threshold_marker_activation")) {
    fail("S-REG-30 lint:fast:inline package wiring is missing.");
  }

  if (!packageJson.scripts?.["test:unit"]?.includes("s_reg_30_sport_metric_extension_threshold_marker_activation_guard.mjs")) {
    fail("S-REG-30 test:unit guard wiring is missing.");
  }
}
...
function assertGeneratedIndexes() {
  ...
  assertIncludes(
    failureTokenIndex,
    "CI_S_REG_30_SPORT_METRIC_EXTENSION_THRESHOLD_MARKER_ACTIVATION",
    "docs/dev/FAILURE_TOKEN_INDEX.md"
  );
}

function assertDocMarkers() {
  const doc = readText("docs/roadmap/S_REG_30_SPORT_METRIC_EXTENSION_THRESHOLD_MARKER_ACTIVATION.md");

  for (const marker of [
    "S-REG-30",
    "sport_metric_extension_threshold_marker_activation",
    "threshold_marker",
    "sport_metric",
    "human-authorised",
    "rollback",
    ...
    assertIncludes(doc, marker, "docs/roadmap/S_REG_30_SPORT_METRIC_EXTENSION_THRESHOLD_MARKER_ACTIVATION.md");
  }
}
...
function assertRequiredProofCommandsPass() {
  for (const [command, ...args] of REQUIRED_PROOF_COMMANDS) {
    ...
    if (typeof result !== "string") {
      fail("S-REG-30 required proof command did not run.", { command, args });
    }
  }
}
...
function main() {
  ...
  if (
    result.ok !== true ||
    result.activation_id !== "sport_metric_extension_threshold_marker_activation" ||
    ...
    result.runtime_status !== S_REG_30_RUNTIME_STATUS
  ) {
    fail("S-REG-30 validation result is invalid.", { result });
  }
  ...
  console.log(JSON.stringify({
    ...
    extended_record_ids: result.extended_record_ids,
