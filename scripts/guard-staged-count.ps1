...
if ($allow -eq "1" -or $allow -eq "true" -or $allow -eq "TRUE") {
  Write-Host "WARN:  staged-count guard bypassed via ALLOW_LARGE_STAGE=$allow" -ForegroundColor Yellow
  exit 0
}
...
if ($count -gt $Max) {
  Write-Host "ERROR: Too many staged files ($count) for a normal commit. Max allowed: $Max" -ForegroundColor Red
...
Write-Host "OK: Staged file count OK ($count <= $Max)" -ForegroundColor Green
