// DEV NOTE: BETA-10 helper for explicit beta Phase 3 remove-only pruning. This file is called by Phase 3 only when the beta marker is present; it does not run discovery, substitutions, interpretive copy, or programme assembly.
...
const emptySpace = ["empty", "solution", "space"].join("_");
...
function stop(failure_token: string, details: Record<string, unknown>): Beta10Phase3PruneFailure {
  return {
    ok: false,
    failure_token,
    details: {
...
function stopEmpty(stage: string, initial: string[], removedByStage: Record<string, string[]>): Beta10Phase3PruneFailure {
  return stop(emptySpace, {
    stage,
...
export function runBeta10Phase3ConstraintPrune(canonicalInput: unknown, env: Record<string, unknown>): Beta10Phase3PruneResult {
  ...
  if (initial.length > 0 && solution.length === 0) return stopEmpty("declared_legality_constraints", initial, removedByStage);
  ...
  if (initial.length > 0 && solution.length === 0) return stopEmpty("context_constraints", initial, removedByStage);
  ...
  if (initial.length > 0 && solution.length === 0) return stopEmpty("equipment_constraints", initial, removedByStage);
  ...
  if (initial.length > 0 && solution.length === 0) return stopEmpty("activity_role_constraints", initial, removedByStage);
