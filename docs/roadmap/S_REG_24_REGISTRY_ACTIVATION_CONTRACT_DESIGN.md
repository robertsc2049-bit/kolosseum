...
- S-REG-29 (`metric_exercise_link_registry` activation) - satisfied this contract's requirements for the `metric_exercise_link` domain only. Unlike every prior activation in this chain, its source candidate had a genuine dangling cross-registry reference (one record's `exercise_id` did not exist in the live exercise registry) - no generic guard checks this, since `s_v1_24`'s FK-closure edge list does not mention this domain. The repository owner explicitly chose to activate only the 12 records with verified-valid references, excluding the dangling one, rather than ship it silently or leave the domain held indefinitely over one bad record.
- S-REG-30 (`sport_metric` extension + `threshold_marker_registry` activation) - satisfied this contract's requirements for the `threshold_marker` domain, plus a content extension of the already-active `sport_metric` domain. Unlike every prior activation in this chain, all 5 of `threshold_marker`'s candidate records referenced `sport_metric_id`s absent from the live registry, traced to a second, never-activated `sport_metric` expansion batch (S-REG-19) that S-REG-27 had not activated. The repository owner explicitly chose to extend `sport_metric` with only the 3 needed records, then activate `threshold_marker`, rather than hold the domain indefinitely. This is the first slice in this chain to mutate an already-active domain's content, which also required relaxing S-REG-27's own exact-count validation to a membership check (documented in S-REG-27's own supersession log).
- S-REG-31 (`exercise_token_registry` activation) - satisfied this contract's requirements for the `exercise_token` domain only. Its dependencies (`activity_registry_1`, `movement_registry_3`) were already legacy-active via S-REG-04's bridge. Like S-REG-29, its source candidate had a genuine dangling cross-registry reference (one record's `movement_id` did not exist in the live movement registry) - no generic guard checks this, since `s_v1_24`'s FK-closure edge list does not mention this domain. The repository owner explicitly chose to activate only the 3 records with verified-valid references, excluding the dangling one.
- S-REG-33 (`exercise_activity_applicability_registry` activation) - satisfied this contract's requirements for the `exercise_activity_applicability` domain only. Unlike every prior activation, its source candidate's 12 records covered only 4 of 19 exercises and only one of the 3 required contexts - genuine closure required authoring 159 records, not converting existing candidate content. This also required evolving `s_v1_23`'s own coverage guard from a must-not-exist block into real enforcement, mirroring `s_v1_22`'s (equipment) identical evolution during S-REG-25, and depended on S-REG-32's prior extension of the `exercise` registry's schema.
