...
import { PLATE_RANK_STYLES, plateVisual, type PlatePair, type WeightUnit } from "../utils/plateCalculator";
...
// diagram at kolosseum.tools/ironclock (renderBarbell()/ic-plate* in that
// repo's IronClock.jsx/.css), adapted from a div/flexbox layout to
// hand-rolled SVG to match this app's own existing visual convention (see
// LineChart.tsx's DEV NOTE - no charting/graphics library). Shows one
// side only, same as the reference: both sides carry the same plates, so a
// second mirrored half would be pure repetition. Rendered at a fixed
// pixel size (not scaled to 100% width) inside a horizontally-scrolling
// wrapper (.barbell-diagram-wrap in styles.css), so a heavy load with
// many plates scrolls instead of shrinking labels into illegibility.
...
const PLATE_GAP = 1;
const PADDING = 16;
const MAX_PLATE_HEIGHT = Math.max(...PLATE_RANK_STYLES.map((style) => style.height));
...
export function BarbellDiagram({ perSide, barWeight, unit }: { perSide: readonly PlatePair[]; barWeight: number; unit: WeightUnit }) {
  ...
  const contentWidth = SHAFT_WIDTH + SLEEVE_SIZE + platesWidth + SLEEVE_END_MARGIN + SLEEVE_END_WIDTH;
  ...
  return (
    <div className="barbell-diagram-wrap">
      <svg
        ...
        role="img"
        aria-label={`Barbell loaded with a ${barWeight}${unit} bar and ${plateCountLabel}`}
      >
        <defs>
