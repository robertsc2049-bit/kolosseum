...
async function runScenario({
  ...
  {
    const pair = await readCachedUncachedPair(baseUrl, sessionId, sessionStateCache, `${label} before start`);
    assertLifecycleStateContract(pair.uncachedState, {
      label: `${label} before start uncached state`,
      expectDecisionRequired: false,
      expectCurrentStep: "EXERCISE"
    });
    assertCacheParity(
  ...
  let postStartPair = await readCachedUncachedPair(baseUrl, sessionId, sessionStateCache, `${label} after start`);
  assertLifecycleStateContract(postStartPair.uncachedState, {
    label: `${label} after start uncached state`,
    expectDecisionRequired: false,
    expectCurrentStep: "EXERCISE"
  });
  assertCacheParity(
    postStartPair.cachedState,
    postStartPair.uncachedState,
    postStartPair.cachedEvents,
    postStartPair.uncachedEvents,
    `${label} after start`
  );
  ...
  const gatedPair = await readCachedUncachedPair(baseUrl, sessionId, sessionStateCache, `${label} gated`);
  assertLifecycleStateContract(gatedPair.uncachedState, {
    label: `${label} gated uncached state`,
    expectDecisionRequired: true,
    expectCurrentStep: "RETURN_DECISION"
  });
  assertCacheParity(
    gatedPair.cachedState,
    gatedPair.uncachedState,
    gatedPair.cachedEvents,
    gatedPair.uncachedEvents,
    `${label} gated`
  );
  ...
  const postDecisionPair = await readCachedUncachedPair(baseUrl, sessionId, sessionStateCache, `${label} post decision`);
  assertLifecycleStateContract(postDecisionPair.uncachedState, {
    label: `${label} post decision uncached state`,
    expectDecisionRequired: false,
    expectCurrentStep: "EXERCISE"
  });
  assertCacheParity(
    postDecisionPair.cachedState,
    postDecisionPair.uncachedState,
    postDecisionPair.cachedEvents,
    postDecisionPair.uncachedEvents,
    `${label} post decision`
  );
  ...
  const terminalPair = await readCachedUncachedPair(baseUrl, sessionId, sessionStateCache, `${label} terminal`);
  assertLifecycleStateContract(terminalPair.uncachedState, {
    label: `${label} terminal uncached state`,
    expectDecisionRequired: false,
    expectCurrentStep: "TERMINAL"
  });
  assertCacheParity(
  ...
  const reloadPair = await readCachedUncachedPair(baseUrl, sessionId, sessionStateCache, `${label} reload`);
...
test("v0 cache parity: compile-created session preserves cached and uncached lifecycle parity (RETURN_CONTINUE)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue cache parity scenario"
    });
  });
});

test("v0 cache parity: compile-created session preserves cached and uncached lifecycle parity (RETURN_SKIP)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip cache parity scenario"
    });
  });
});