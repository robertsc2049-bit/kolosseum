...
test("package.json matches pinned dev session API CI manifest", () => {
  const repo = process.cwd();
  const pkgPath = path.join(repo, "package.json");
  const manifestPath = path.join(repo, "ci", "contracts", "dev_session_api_ci_cluster.json");
  ...
  assert.ok(cluster.length > 0, "expected non-empty dev session API cluster manifest");
  ...
  assert.deepEqual(
    ...
    cluster,
    "expected dev session API manifest cluster to be adjacent with no inserted commands"
  );
});