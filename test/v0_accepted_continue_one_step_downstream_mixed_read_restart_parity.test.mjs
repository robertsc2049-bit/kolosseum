...
function assertContinuePathStateAfterOneDownstreamStep(
  statePayload,
  ...
  assert.ok(
    ...
    `${label}: expected nonterminal remaining_ids after one downstream completion.\ntrace=${JSON.stringify(trace)}`
  );

  assert.equal(
    currentExerciseId,
    expectedCurrentExerciseId,
    `${label}: current_step drifted.\nstate=${JSON.stringify(state)}`
  );
...
test(
  "test(v0): prove accepted continue path preserves mixed-read restart parity after one downstream completion",
  async (t) => {
    ...
    assert.ok(
      ...
      `expected at least two remaining ids for one-step downstream continue path.\ntrace=${JSON.stringify(splitState.json?.trace)}`
    );
    assert.equal(
      splitRemainingIds[0],
      secondExerciseId,
    ...
    const stateAfterDownstreamComplete = await httpJson(
      "GET",
      `${http.baseUrl}/sessions/${sessionId}/state`
    );
    assert.equal(
      stateAfterDownstreamComplete.res.status,
      200,
      `state after downstream completion expected 200, got ${stateAfterDownstreamComplete.res.status}. raw=${stateAfterDownstreamComplete.text}`
    );

    const thirdExerciseId = stateAfterDownstreamComplete.json?.current_step?.exercise?.exercise_id ?? null;
    assert.ok(
      typeof thirdExerciseId === "string" && thirdExerciseId.length > 0,
      `expected third exercise id after one downstream completion.\nstate=${stateAfterDownstreamComplete.text}`
    );
    assert.notEqual(
      thirdExerciseId,
      secondExerciseId,
      `expected downstream progress to advance beyond resumed second exercise.\nstate=${stateAfterDownstreamComplete.text}`
    );

    const expectedCompletedIds = [firstExerciseId, secondExerciseId];

    assertContinuePathStateAfterOneDownstreamStep(
      stateAfterDownstreamComplete,
      expectedCompletedIds,
      thirdExerciseId,
      "state after accepted continue + one downstream completion"
    );
    ...
    assert.notEqual(
      ...
      201,
      `replayed RETURN_SKIP must be rejected after accepted continue + downstream progress. raw=${replaySkipRejected.text}`
    );
    assert.ok(
    ...
    assert.deepEqual(
      ...
      [1, 2, 3, 4, 5],
      `expected seq [1,2,3,4,5], got ${JSON.stringify(warmCycle.events1.json.events)}`
    );
    assert.deepEqual(
      ...
      `unexpected event order after accepted continue + one downstream completion.\ngot ${JSON.stringify(warmCycle.events1.json.events)}`
    );
