...
test("CI wrapper: planSession direct-body and wrapped-input success parity passes with experimental module mocks", () => {
  const repo = process.cwd();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kolosseum-plan-session-success-parity-"));
  const target = path.join(tempDir, "api_plan_session_success_parity_runtime_contract.test.mjs");
  ...
  const lines = [
    ...
    '',
    'test("planSession runtime success parity: direct body and wrapped input produce identical handler-edge status and payload", async () => {',
    ...
    '',
    '  const directReq = makeReq(equivalentInput);',
    '  const wrappedReq = makeReq({ input: equivalentInput });',
    '  const directRes = makeRes();',
    '  const wrappedRes = makeRes();',
    '',
    '  await planSession(directReq, directRes);',
    '  await planSession(wrappedReq, wrappedRes);',
    '',
    '  assert.deepEqual(planSessionServiceCalls, [equivalentInput, equivalentInput]);',
    '  assert.deepEqual(badRequestCalls, []);',
    '',
    '  assert.equal(directRes.statusCode, 200);',
    '  assert.equal(wrappedRes.statusCode, 200);',
    '  assert.deepEqual(directRes.jsonBody, wrappedRes.jsonBody);',
