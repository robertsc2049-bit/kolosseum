...
function assertTerminalContinuePathState(
  statePayload,
...
test(
  "test(v0): prove accepted continue path preserves terminal no-resurrection invariants across mixed-read restart parity",
  async (t) => {
    ...
    assert.ok(
      ...
      `expected at least two remaining ids for terminal continue path proof.\ntrace=${JSON.stringify(splitState.json?.trace)}`
    );
    assert.equal(
      splitRemainingIds[0],
      gatedExerciseId,
    ...
    let terminalState = stateAfterContinue;
    ...
    assert.equal(
      ...
      true,
      `expected terminal state after exhausting remaining work on continue path.\nstate=${terminalState.text}`
    );

    const expectedCompletedIds = cloneJson(completedIds);

    assert.ok(
      expectedCompletedIds.length >= 3,
      `expected continue path to include initial completion, resumed gated completion, and at least one later completion.\ncompleted=${JSON.stringify(expectedCompletedIds)}`
    );
    assert.equal(
      expectedCompletedIds.includes(gatedExerciseId),
      true,
      `continued gated exercise must appear in completed ids.\ncompleted=${JSON.stringify(expectedCompletedIds)}`
    );

    assertTerminalContinuePathState(
      terminalState,
      expectedCompletedIds,
      "terminal state after accepted continue"
    );
    ...
    assert.notEqual(
      ...
      201,
      `replayed RETURN_CONTINUE must be rejected after terminal continue path. raw=${replayContinueRejected.text}`
    );
    assert.ok(
    ...
    assert.notEqual(
      ...
      201,
      `replayed RETURN_SKIP must be rejected after terminal continue path. raw=${replaySkipRejected.text}`
    );
    assert.ok(
    ...
    assert.notEqual(
      ...
      201,
      `continued gated exercise must not be completable again after terminalization. raw=${replayGatedCompletionRejected.text}`
    );
    assert.ok(
      [400, 409, 422].includes(replayGatedCompletionRejected.res.status),
      `replayed COMPLETE_EXERCISE for continued item expected 400/409/422, got ${replayGatedCompletionRejected.res.status}. raw=${replayGatedCompletionRejected.text}`
    );
    ...
    assert.deepEqual(
      ...
      `unexpected event prefix for terminal continue path.\ngot ${JSON.stringify(warmCycle.events1.json.events)}`
    );
