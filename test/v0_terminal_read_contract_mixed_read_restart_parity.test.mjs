...
function assertTerminalStateContract(
  statePayload,
...
test(
  "test(v0): prove terminal state read contract is byte-stable across mixed reads and restart hydration",
  async (t) => {
    ...
    assert.ok(
      ...
      `expected at least two remaining ids for terminal read-contract path.\ntrace=${JSON.stringify(splitState.json?.trace)}`
    );
    assert.equal(
      splitRemainingIds[0],
      secondExerciseId,
