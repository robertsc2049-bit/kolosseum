...
test("package.json matches pinned canonical hash API CI manifest", () => {
  const repo = process.cwd();
  const pkgPath = path.join(repo, "package.json");
  const manifestPath = path.join(repo, "ci", "contracts", "canonical_hash_api_ci_cluster.json");
  ...
  assert.ok(cluster.length > 0, "expected non-empty canonical hash API cluster manifest");
  ...
  assert.deepEqual(
    ...
    cluster,
    "expected canonical hash API manifest cluster to be adjacent with no inserted commands"
  );
});