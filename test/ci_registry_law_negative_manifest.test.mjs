...
test("package.json matches pinned registry law negative CI manifest", () => {
  const repo = process.cwd();
  const pkgPath = path.join(repo, "package.json");
  const manifestPath = path.join(repo, "ci", "contracts", "registry_law_negative_ci_cluster.json");
  ...
  assert.ok(cluster.length > 0, "expected non-empty registry law negative cluster manifest");
  ...
  assert.deepEqual(
    ...
    cluster,
    "expected registry law negative manifest cluster to be adjacent with no inserted commands"
  );
});