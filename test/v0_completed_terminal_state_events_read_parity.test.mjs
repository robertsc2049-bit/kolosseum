...
test(
  "test(v0): completed terminal state and events remain parity-stable across repeated read cycles",
  async (t) => {
    ...
    assert.equal(
      ...
      "completed",
      `state-A: expected completed terminal state.\nprojection=${JSON.stringify(stateA.projection)}`
    );
    assert.equal(
      stateA.projection.trace.dropped_ids.length,
      0,
      `state-A: completed path must not drop work.\nprojection=${JSON.stringify(stateA.projection)}`
    );
    assert.equal(
      stateA.projection.trace.return_decision_required,
      false,
      `state-A: completed path must not require return decision.\nprojection=${JSON.stringify(stateA.projection)}`
    );

    assert.deepEqual(
      stateA.projection,
      stateB.projection,
      `completed terminal state must be parity-stable across repeated reads.\nA=${JSON.stringify(stateA.projection)}\nB=${JSON.stringify(stateB.projection)}`
    );
    assert.deepEqual(
      stateB.projection,
      stateC.projection,
      `completed terminal state must remain identical across third read.\nB=${JSON.stringify(stateB.projection)}\nC=${JSON.stringify(stateC.projection)}`
    );

    assert.deepEqual(
      eventsA.projection,
      eventsB.projection,
      `completed terminal events must be parity-stable across repeated reads.\nA=${JSON.stringify(eventsA.projection)}\nB=${JSON.stringify(eventsB.projection)}`
    );
    assert.deepEqual(
      eventsB.projection,
      eventsC.projection,
      `completed terminal events must remain identical across third read.\nB=${JSON.stringify(eventsB.projection)}\nC=${JSON.stringify(eventsC.projection)}`
    );
  }
);