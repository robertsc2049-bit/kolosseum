...
async function runScenario({
  ...
  try {
    ...
    const preProgressExerciseId =
      freshBefore.uncachedState.json.current_step?.exercise?.exercise_id ?? null;

    assert.equal(
      preProgressExerciseId,
      baselineExerciseId,
    ...
    await completeCurrentExercise(
      ...
      sessionId,
      freshBefore.uncachedState,
      `${label} fresh valid progress`
    );

    const freshAfter = await captureLivePair(
      freshServer.baseUrl,
      sessionId,
      freshServer.sessionStateCache,
      `${label} fresh after progress`
    );
    assertLivePairStable(freshAfter, { label: `${label} fresh after progress` });

    const postProgressExerciseId =
      freshAfter.uncachedState.json.current_step?.exercise?.exercise_id ?? null;
    ...
    assert.notEqual(
      ...
      preProgressExerciseId,
      `${label}: live current_step resurrected stale pre-progress exercise after fresh restart write`
    );

    assert.equal(
      freshAfter.uncachedEvents.json.events.length,
      freshBefore.uncachedEvents.json.events.length + 1,
      `${label}: expected exactly one appended runtime event after valid progress`
    );

    const freshAfterSnapshot = snapshotEvents(freshAfter.uncachedEvents);
    for (let i = 0; i < baselineSnapshot.length; i += 1) {
      assert.deepEqual(
        freshAfterSnapshot[i],
        baselineSnapshot[i],
        `${label}: append-only event prefix drifted at index ${i}`
      );
    }

    const reloadAfter = await captureLivePair(
      freshServer.baseUrl,
      sessionId,
      freshServer.sessionStateCache,
      `${label} reload after progress`
    );
    assertLivePairStable(reloadAfter, { label: `${label} reload after progress` });

    assertByteStableState(
      reloadAfter.uncachedState,
      freshAfter.uncachedState,
      `${label}: reload-after-progress uncached state parity`
    );
    assertByteStableEvents(
      reloadAfter.uncachedEvents,
      freshAfter.uncachedEvents,
      `${label}: reload-after-progress uncached events parity`
    );
    assertEventIdentityStable(
      reloadAfter.uncachedEvents,
      freshAfterSnapshot,
      `${label}: reload-after-progress event identity parity`
    );
  } finally {
...
test("v0 fresh restart live progress: compile-created live session advances coherently after fresh restart (RETURN_CONTINUE)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue fresh restart live progress coherence scenario"
    });
  });
});

test("v0 fresh restart live progress: compile-created live session advances coherently after fresh restart (RETURN_SKIP)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip fresh restart live progress coherence scenario"
    });
  });
});