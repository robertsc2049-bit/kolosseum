...
test(
  "test(v0): completed and partial terminal restart mixed-read matrix remains parity-stable after fresh process boot",
  async (t) => {
    ...
    const completedBaselineState = await getState(
      firstHttp.baseUrl,
      completedSessionId,
      "completed-baseline-state"
    );
    const completedBaselineEvents = await getEvents(
      firstHttp.baseUrl,
      completedSessionId,
      "completed-baseline-events"
    );

    const partialBaselineState = await getState(
      firstHttp.baseUrl,
      partialSessionId,
      "partial-baseline-state"
    );
    const partialBaselineEvents = await getEvents(
      firstHttp.baseUrl,
      partialSessionId,
      "partial-baseline-events"
    );

    assertCompletedTerminalSemantics(
      completedBaselineState.projection,
      "completed-baseline-state"
    );
    assertPartialTerminalSemantics(
      partialBaselineState.projection,
      "partial-baseline-state"
    );
    ...
    const completedSeq1 = await readMixedStateEventsState(
      secondHttp.baseUrl,
      completedSessionId,
      "completed-seq1-after-restart"
    );
    const completedSeq2 = await readMixedEventsStateEvents(
      secondHttp.baseUrl,
      completedSessionId,
      "completed-seq2-after-restart"
    );
    const completedSeq3 = await readMixedStateEventsStateEvents(
      secondHttp.baseUrl,
      completedSessionId,
      "completed-seq3-after-restart"
    );
    const completedSeq4 = await readMixedEventsStateEventsState(
      secondHttp.baseUrl,
      completedSessionId,
      "completed-seq4-after-restart"
    );

    const partialSeq1 = await readMixedStateEventsState(
      secondHttp.baseUrl,
      partialSessionId,
      "partial-seq1-after-restart"
    );
    const partialSeq2 = await readMixedEventsStateEvents(
      secondHttp.baseUrl,
      partialSessionId,
      "partial-seq2-after-restart"
    );
    const partialSeq3 = await readMixedStateEventsStateEvents(
      secondHttp.baseUrl,
      partialSessionId,
      "partial-seq3-after-restart"
    );
    const partialSeq4 = await readMixedEventsStateEventsState(
      secondHttp.baseUrl,
      partialSessionId,
      "partial-seq4-after-restart"
    );

    assertCompletedTerminalSemantics(
      completedSeq1.stateA.projection,
      "completed-seq1-after-restart-state-A"
    );
    assertCompletedTerminalSemantics(
      completedSeq2.stateA.projection,
      "completed-seq2-after-restart-state-A"
    );
    assertCompletedTerminalSemantics(
      completedSeq3.stateA.projection,
      "completed-seq3-after-restart-state-A"
    );
    assertCompletedTerminalSemantics(
      completedSeq4.stateA.projection,
      "completed-seq4-after-restart-state-A"
    );

    assertPartialTerminalSemantics(
      partialSeq1.stateA.projection,
      "partial-seq1-after-restart-state-A"
    );
    assertPartialTerminalSemantics(
      partialSeq2.stateA.projection,
      "partial-seq2-after-restart-state-A"
    );
    assertPartialTerminalSemantics(
      partialSeq3.stateA.projection,
      "partial-seq3-after-restart-state-A"
    );
    assertPartialTerminalSemantics(
      partialSeq4.stateA.projection,
      "partial-seq4-after-restart-state-A"
    );

    assertMixedReadParity(completedSeq1, "completed-seq1-after-restart");
    assertMixedReadParity(completedSeq2, "completed-seq2-after-restart");
    assertMixedReadParity(completedSeq3, "completed-seq3-after-restart");
    assertMixedReadParity(completedSeq4, "completed-seq4-after-restart");

    assertMixedReadParity(partialSeq1, "partial-seq1-after-restart");
    assertMixedReadParity(partialSeq2, "partial-seq2-after-restart");
    assertMixedReadParity(partialSeq3, "partial-seq3-after-restart");
    assertMixedReadParity(partialSeq4, "partial-seq4-after-restart");

    assert.deepEqual(
      completedBaselineState.projection,
      completedSeq1.stateA.projection,
      `completed terminal state must survive restart and mixed reads without drift.\nBEFORE=${JSON.stringify(completedBaselineState.projection)}\nAFTER=${JSON.stringify(completedSeq1.stateA.projection)}`
    );
    assert.deepEqual(
      completedBaselineEvents.projection,
      completedSeq1.eventsA.projection,
      `completed terminal events must survive restart and mixed reads without drift.\nBEFORE=${JSON.stringify(completedBaselineEvents.projection)}\nAFTER=${JSON.stringify(completedSeq1.eventsA.projection)}`
    );

    assert.deepEqual(
      partialBaselineState.projection,
      partialSeq1.stateA.projection,
      `partial terminal state must survive restart and mixed reads without drift.\nBEFORE=${JSON.stringify(partialBaselineState.projection)}\nAFTER=${JSON.stringify(partialSeq1.stateA.projection)}`
    );
    assert.deepEqual(
      partialBaselineEvents.projection,
      partialSeq1.eventsA.projection,
      `partial terminal events must survive restart and mixed reads without drift.\nBEFORE=${JSON.stringify(partialBaselineEvents.projection)}\nAFTER=${JSON.stringify(partialSeq1.eventsA.projection)}`
    );

    assert.deepEqual(
      completedSeq1.stateA.projection,
      completedSeq2.stateA.projection,
      `completed restart mixed-read matrix must preserve identical state across sequence variants.\nSEQ1=${JSON.stringify(completedSeq1.stateA.projection)}\nSEQ2=${JSON.stringify(completedSeq2.stateA.projection)}`
    );
    assert.deepEqual(
      completedSeq1.stateA.projection,
      completedSeq3.stateA.projection,
      `completed restart mixed-read matrix must preserve identical state across longer sequence variants.\nSEQ1=${JSON.stringify(completedSeq1.stateA.projection)}\nSEQ3=${JSON.stringify(completedSeq3.stateA.projection)}`
    );
    assert.deepEqual(
      completedSeq1.stateA.projection,
      completedSeq4.stateA.projection,
      `completed restart mixed-read matrix must preserve identical state across events-first sequence variants.\nSEQ1=${JSON.stringify(completedSeq1.stateA.projection)}\nSEQ4=${JSON.stringify(completedSeq4.stateA.projection)}`
    );

    assert.deepEqual(
      completedSeq1.eventsA.projection,
      completedSeq2.eventsA.projection,
      `completed restart mixed-read matrix must preserve identical events across sequence variants.\nSEQ1=${JSON.stringify(completedSeq1.eventsA.projection)}\nSEQ2=${JSON.stringify(completedSeq2.eventsA.projection)}`
    );
    assert.deepEqual(
      completedSeq1.eventsA.projection,
      completedSeq3.eventsA.projection,
      `completed restart mixed-read matrix must preserve identical events across longer sequence variants.\nSEQ1=${JSON.stringify(completedSeq1.eventsA.projection)}\nSEQ3=${JSON.stringify(completedSeq3.eventsA.projection)}`
    );
    assert.deepEqual(
      completedSeq1.eventsA.projection,
      completedSeq4.eventsA.projection,
      `completed restart mixed-read matrix must preserve identical events across events-first sequence variants.\nSEQ1=${JSON.stringify(completedSeq1.eventsA.projection)}\nSEQ4=${JSON.stringify(completedSeq4.eventsA.projection)}`
    );

    assert.deepEqual(
      partialSeq1.stateA.projection,
      partialSeq2.stateA.projection,
      `partial restart mixed-read matrix must preserve identical state across sequence variants.\nSEQ1=${JSON.stringify(partialSeq1.stateA.projection)}\nSEQ2=${JSON.stringify(partialSeq2.stateA.projection)}`
    );
    assert.deepEqual(
      partialSeq1.stateA.projection,
      partialSeq3.stateA.projection,
      `partial restart mixed-read matrix must preserve identical state across longer sequence variants.\nSEQ1=${JSON.stringify(partialSeq1.stateA.projection)}\nSEQ3=${JSON.stringify(partialSeq3.stateA.projection)}`
    );
    assert.deepEqual(
      partialSeq1.stateA.projection,
      partialSeq4.stateA.projection,
      `partial restart mixed-read matrix must preserve identical state across events-first sequence variants.\nSEQ1=${JSON.stringify(partialSeq1.stateA.projection)}\nSEQ4=${JSON.stringify(partialSeq4.stateA.projection)}`
    );

    assert.deepEqual(
      partialSeq1.eventsA.projection,
      partialSeq2.eventsA.projection,
      `partial restart mixed-read matrix must preserve identical events across sequence variants.\nSEQ1=${JSON.stringify(partialSeq1.eventsA.projection)}\nSEQ2=${JSON.stringify(partialSeq2.eventsA.projection)}`
    );
    assert.deepEqual(
      partialSeq1.eventsA.projection,
      partialSeq3.eventsA.projection,
      `partial restart mixed-read matrix must preserve identical events across longer sequence variants.\nSEQ1=${JSON.stringify(partialSeq1.eventsA.projection)}\nSEQ3=${JSON.stringify(partialSeq3.eventsA.projection)}`
    );
    assert.deepEqual(
      partialSeq1.eventsA.projection,
      partialSeq4.eventsA.projection,
      `partial restart mixed-read matrix must preserve identical events across events-first sequence variants.\nSEQ1=${JSON.stringify(partialSeq1.eventsA.projection)}\nSEQ4=${JSON.stringify(partialSeq4.eventsA.projection)}`
    );
  }
);