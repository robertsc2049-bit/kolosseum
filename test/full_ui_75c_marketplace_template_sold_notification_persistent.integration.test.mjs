// DEV NOTE: FULL-UI-75 marketplace-template-sold notification persistent
// proof. Proves that releasing a template to a buyer creates exactly one
// notification for the SELLER (never the buyer's own release
// notification confused with it), correctly deep-linking to the
// seller's own original template, that it starts unread and can be
// marked read, that repeated reads never duplicate the derived
// notification, that a second sale (to a different buyer) produces a
...
async function registerCoach(baseUrl, nonce, label) {
  const email = `sold_notif_${label}_coach_${nonce}@example.com`;
  const result = await request(baseUrl, "POST", "/account/register", {
    actor_type: "coach",
    display_name: `Sold Notif ${label} Coach`,
    email,
    password: `SoldNotif${label}Coach!2026`,
    accepted_terms: true,
  ...
  assertStatus(await request(
    ...
    { display_name: `Sold Notif ${label} Coach`, email },
    { cookie, csrf }
...
test(
  "Marketplace-sold notification: exactly one notification for the seller only, never for the buyer's own release notification, correct deep link, starts unread, mark-read works, repeated reads never duplicate, a second sale produces a second independent notification, deterministic compile untouched, fresh-process restart",
  async (testContext) => {
    const root = repoRoot();
    ...
    assertStatus(release, 201, "coach A sells to coach B");

    // ============================================================
    // The seller sees exactly one marketplace-sold notification,
    // unread, correctly deep-linking to their own original template,
    // with the buyer's identity and the sold template_id as payload.
    // ============================================================
    ...
    const soldNotifications = sellerNotifications.json.notifications.filter(
      (entry) => entry.notification_type === "marketplace_template_sold"
    );
    assert.equal(soldNotifications.length, 1, "expected exactly one sold notification for coach A");
    const notification = soldNotifications[0];
    assert.equal(notification.deep_link.route_id, "coach_programme_detail");
    assert.equal(notification.deep_link.params.template_id, templateId);
    assert.equal(notification.target_available, true);
    assert.equal(notification.notification_payload.buyer_coach_user_id, coachB.userId);
    ...
    // ============================================================
    // The buyer never sees a marketplace_template_sold notification
    // themselves - only their own (different) release notification.
    // ============================================================
    ...
    assert.equal(
      buyerNotifications.json.notifications.some((entry) => entry.notification_type === "marketplace_template_sold"),
      false,
      "expected the buyer to never receive the seller's sold notification"
    );
    assert.equal(
      buyerNotifications.json.notifications.some((entry) => entry.notification_type === "marketplace_template_released"),
      true,
      "expected the buyer to still receive their own pre-existing release notification"
    );

    // ============================================================
    // Marking it read persists, and repeated reads of the seller's
    ...
    const sellerNotificationsAfterRead = await request(baseUrl, "GET", "/account/notifications", undefined, { cookie: coachA.cookie });
    const soldNotificationsAfterRead = sellerNotificationsAfterRead.json.notifications.filter(
      (entry) => entry.notification_type === "marketplace_template_sold"
    );
    assert.equal(soldNotificationsAfterRead.length, 1, "expected still exactly one sold notification after repeated reads");
    assert.notEqual(soldNotificationsAfterRead[0].read_at_iso8601, null, "expected the notification to now be read");

    // ============================================================
    // A second sale, to a different buyer, produces a second,
    // independent notification for the seller.
    // ============================================================
    const secondRelease = await request(
      baseUrl, "POST", `/programme-marketplace/templates/${encodeURIComponent(templateId)}/release`,
      { buyer_account_code: coachC.userId }, { cookie: coachA.cookie, csrf: coachA.csrf }
    );
    assertStatus(secondRelease, 201, "coach A sells the same template to coach C too");

    const sellerNotificationsAfterSecond = await request(baseUrl, "GET", "/account/notifications", undefined, { cookie: coachA.cookie });
    const soldNotificationsAfterSecond = sellerNotificationsAfterSecond.json.notifications.filter(
      (entry) => entry.notification_type === "marketplace_template_sold"
    );
    assert.equal(soldNotificationsAfterSecond.length, 2, "expected a second, independent sold notification for coach A");
    assert.ok(
      soldNotificationsAfterSecond.some((entry) => entry.notification_payload.buyer_coach_user_id === coachC.userId),
      "expected the second notification to carry the second buyer's identity"
    );
    ...
    assert.deepEqual(compileAfter, compileBefore, "Marketplace-sold notifications read altered deterministic compile output.");
    ...
    const restartedSoldNotifications = restartedNotifications.json.notifications.filter(
      (entry) => entry.notification_type === "marketplace_template_sold"
    );
    assert.equal(restartedSoldNotifications.length, 2);
    assert.ok(restartedSoldNotifications.some((entry) => entry.read_at_iso8601 !== null));
  }
);
