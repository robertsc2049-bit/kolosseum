...
test("package.json matches pinned plan registry contract CI manifest", () => {
  const repo = process.cwd();
  const pkgPath = path.join(repo, "package.json");
  const manifestPath = path.join(repo, "ci", "contracts", "plan_registry_contract_ci_cluster.json");
  ...
  assert.ok(cluster.length > 0, "expected non-empty plan registry contract cluster manifest");
  ...
  assert.deepEqual(
    ...
    cluster,
    "expected plan registry contract manifest cluster to be adjacent with no inserted commands"
  );
});