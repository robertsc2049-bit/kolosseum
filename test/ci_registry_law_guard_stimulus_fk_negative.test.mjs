...
test("CI: registry_law_guard hard-fails on FK break (exercise.stimulus_intent -> not in activity stimulus_intents)", () => {
  ...
  try {
    ...
    assert.equal(typeof original.entries[k0].stimulus_intent, "string", "expected entry.stimulus_intent string");

    original.entries[k0].stimulus_intent = "__fk_break_nonexistent_stimulus__";
    ...
    assert.match(combined, /FK fail stimulus_intent/i);
  } finally {
