...
async function captureStateEventsPair(baseUrl, sessionId, sessionStateCache, label) {
  sessionStateCache.clear();
...
function assertTerminalPairStable(pair, { label }) {
...
async function runScenario({
  ...
  const baseline = await captureStateEventsPair(baseUrl, sessionId, sessionStateCache, `${label} baseline`);
  assertTerminalPairStable(baseline, { label: `${label} baseline` });

  const baselineSnapshot = snapshotEvents(baseline.uncachedEvents);

  const freshServer = await startFreshServer(root, databaseUrl);
  try {
    const freshBefore = await captureStateEventsPair(
      freshServer.baseUrl,
      sessionId,
      freshServer.sessionStateCache,
      `${label} fresh before rejected writes`
    );
    assertTerminalPairStable(freshBefore, { label: `${label} fresh before rejected writes` });
    ...
    const rejectedComplete = await httpJson(
      "POST",
    ...
    assertRejectedMutation(rejectedComplete, `${label} rejected complete after restart`);

    const rejectedDecisionReplay = await httpJson(
      "POST",
      `${freshServer.baseUrl}/sessions/${sessionId}/events`,
      { event: { type: decisionType } }
    );
    assertRejectedMutation(rejectedDecisionReplay, `${label} rejected decision replay after restart`);

    const rejectedSplitReplay = await httpJson(
      "POST",
      `${freshServer.baseUrl}/sessions/${sessionId}/events`,
      { event: { type: "SPLIT_SESSION" } }
    );
    assertRejectedMutation(rejectedSplitReplay, `${label} rejected split replay after restart`);

    const freshAfter = await captureStateEventsPair(
      freshServer.baseUrl,
      sessionId,
      freshServer.sessionStateCache,
      `${label} fresh after rejected writes`
    );
    assertTerminalPairStable(freshAfter, { label: `${label} fresh after rejected writes` });

    assertByteStableState(
      freshAfter.uncachedState,
      freshBefore.uncachedState,
      `${label}: fresh-after uncached state parity`
    );
    assertByteStableEvents(
      freshAfter.uncachedEvents,
      freshBefore.uncachedEvents,
      `${label}: fresh-after uncached events parity`
    );
    assertByteStableState(
      freshAfter.cachedState,
      freshBefore.cachedState,
      `${label}: fresh-after cached state parity`
    );
    assertByteStableEvents(
      freshAfter.cachedEvents,
      freshBefore.cachedEvents,
      `${label}: fresh-after cached events parity`
    );
    assertEventIdentityStable(
      freshAfter.uncachedEvents,
      baselineSnapshot,
      `${label}: fresh-after event identity parity`
    );

    assertByteStableState(
      freshAfter.uncachedState,
      baseline.uncachedState,
      `${label}: post-rejection state still matches baseline terminal snapshot`
    );
    assertByteStableEvents(
      freshAfter.uncachedEvents,
      baseline.uncachedEvents,
      `${label}: post-rejection events still match baseline terminal snapshot`
    );
  } finally {
...
test("v0 terminal restart closure: compile-created session remains mutation-closed after fresh restart (RETURN_CONTINUE)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue terminal restart mutation closure scenario"
    });
  });
});

test("v0 terminal restart closure: compile-created session remains mutation-closed after fresh restart (RETURN_SKIP)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip terminal restart mutation closure scenario"
    });
  });
});