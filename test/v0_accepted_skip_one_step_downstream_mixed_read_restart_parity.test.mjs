...
function assertSkipPathStateAfterOneDownstreamStep(
  statePayload,
  ...
  assert.equal(
    currentExerciseId,
    remainingIds[0],
    `${label}: expected current_step to align with remaining_ids[0].\nstate=${JSON.stringify(state)}`
  );
  ...
  for (const droppedId of expectedDroppedIds) {
    ...
    assert.notEqual(
      currentExerciseId,
      droppedId,
      `${label}: current_step must not resurrect dropped id ${droppedId}.\nstate=${JSON.stringify(state)}`
    );
  }
}
...
test(
  "test(v0): prove accepted skip path preserves mixed-read restart parity after one downstream completion",
  async (t) => {
    ...
    assert.ok(
      ...
      `expected at least two remaining ids for one-step downstream skip path.\ntrace=${JSON.stringify(splitState.json?.trace)}`
    );
    assert.equal(
      splitRemainingIds[0],
      secondExerciseId,
    ...
    const expectedCompletedIds = [firstExerciseId, resumedExerciseId];
    const expectedDroppedIds = [secondExerciseId];

    assertSkipPathStateAfterOneDownstreamStep(
      stateAfterDownstreamComplete,
      expectedCompletedIds,
      expectedDroppedIds,
      "state after accepted skip + one downstream completion"
    );
    ...
    assert.notEqual(
      ...
      201,
      `replayed RETURN_CONTINUE must be rejected after accepted skip + downstream progress. raw=${replayContinueRejected.text}`
    );
    assert.ok(
    ...
    assert.deepEqual(
      ...
      ["START_SESSION", "COMPLETE_EXERCISE", "SPLIT_SESSION", "RETURN_SKIP", "COMPLETE_EXERCISE"],
      `unexpected event order after accepted skip + one downstream completion.\ngot ${JSON.stringify(warmCycle.events1.json.events)}`
    );
