// DEV NOTE: FULL-UI-35 - exercise coaching-content lookup proof. Proves both
// an athlete and a coach can read the same route, a real exercise returns
// real non-empty written instructions/cues/faults, an unknown exercise_id
// returns 404, an unauthenticated request is rejected, and the route
// survives a fresh process restart. Every step crosses only public HTTP
// routes; this feature touches no database table of its own, so
// restart-survival here proves the route is a genuinely deployed endpoint,
// not an in-memory-only artefact.
...
async function registerCoach(baseUrl, nonce) {
  const email = `exercisecontent_coach_${nonce}@example.com`;
  const result = await request(baseUrl, "POST", "/account/register", {
    actor_type: "coach",
    display_name: "Exercise Content Coach",
    email,
    password: "ExerciseContentCoach!2026",
    accepted_terms: true,
  ...
  const onboardingProfile = await request(
    ...
    { display_name: "Exercise Content Coach", email },
    { cookie, csrf }
  );
...
async function registerAthlete(baseUrl, nonce) {
  const email = `exercisecontent_athlete_${nonce}@example.com`;
  const result = await request(baseUrl, "POST", "/account/register", {
    actor_type: "athlete",
    display_name: "Exercise Content Athlete",
    email,
    password: "ExerciseContentAthlete!2026",
...
test(
  "Exercise content: athlete and coach both read the same real written instructions, unknown exercise 404s, unauthenticated is rejected, survives a fresh-process restart",
  async (testContext) => {
    const root = repoRoot();
    ...
    // ============================================================
    // A real exercise returns real, non-empty written instructions,
    // coaching cues and common faults. Both actor types read the
    // identical route and get the identical response shape.
    // ============================================================
    const athleteLookup = await request(
      baseUrl, "GET", "/exercises/back_squat/content", undefined, { cookie: athlete.cookie }
    );
    assertStatus(athleteLookup, 200, "athlete reads exercise content");
    assert.equal(athleteLookup.json?.ok, true);
    assert.equal(athleteLookup.json?.exercise_id, "back_squat");
    assert.ok(athleteLookup.json?.instruction?.short?.length > 0, "expected a non-empty instruction.short");
    assert.ok(
      Array.isArray(athleteLookup.json?.instruction?.detailed) && athleteLookup.json.instruction.detailed.length > 0,
      "expected non-empty instruction.detailed"
    );
    assert.ok(
      Array.isArray(athleteLookup.json?.coaching_cues) && athleteLookup.json.coaching_cues.length > 0,
      "expected non-empty coaching_cues"
    );
    assert.ok(
      Array.isArray(athleteLookup.json?.common_faults) && athleteLookup.json.common_faults.length > 0,
      "expected non-empty common_faults"
    );

    const coachLookup = await request(
      baseUrl, "GET", "/exercises/back_squat/content", undefined, { cookie: coach.cookie }
    );
    assertStatus(coachLookup, 200, "coach reads the identical exercise content route");
    assert.equal(coachLookup.json?.exercise_id, "back_squat");
    assert.deepEqual(coachLookup.json?.coaching_cues, athleteLookup.json?.coaching_cues);
    assert.deepEqual(coachLookup.json?.common_faults, athleteLookup.json?.common_faults);

    // ============================================================
    // An unknown exercise_id 404s rather than silently returning empty
    // arrays - the distinction between "no content yet" and "not a real
    // exercise".
    // ============================================================
    const unknownLookup = await request(
      baseUrl, "GET", "/exercises/not_a_real_exercise_id/content", undefined, { cookie: athlete.cookie }
    );
    ...
    const unauthenticatedLookup = await request(
      baseUrl, "GET", "/exercises/back_squat/content", undefined, {}
    );
    ...
    const restartedLookup = await request(
      restarted.baseUrl, "GET", "/exercises/back_squat/content", undefined, { cookie: athlete.cookie }
    );
    assertStatus(restartedLookup, 200, "exercise content lookup after fresh-process restart");
    assert.equal(restartedLookup.json?.exercise_id, "back_squat");
    assert.ok(restartedLookup.json?.coaching_cues?.length > 0, "expected non-empty coaching_cues after restart");
  }
);
