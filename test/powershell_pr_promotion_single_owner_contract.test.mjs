...
test("powershell PR promotion helper source pins single-owner JSON-only checks path", () => {
  ...
  assert.match(source, /\$script:PSNativeCommandUseErrorActionPreference = \$false/);
  ...
  assert.match(source, /PR_CHECKS_PENDING/);
  assert.match(source, /PR_CHECKS_GREEN/);
  assert.match(source, /PR_CHECKS_FAIL/);
