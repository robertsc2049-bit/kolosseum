...
test("CI wrapper: planSession invalid body runtime contract passes with experimental module mocks", () => {
  const repo = process.cwd();
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "kolosseum-plan-session-invalid-body-"));
  const target = path.join(tempDir, "api_plan_session_invalid_body_runtime_contract.test.mjs");
  ...
  const lines = [
    ...
    '',
    'test("planSession runtime boundary rejects non-object JSON body before service delegation", async () => {',
    ...
    '    () => planSession(req, res),',
    '    (error) => {',
    '      assert.equal(error?.message, "Invalid JSON body (expected object)");',
    '      assert.equal(error?.status, 400);',
    '      return true;',
    '    }',
    '  );',
    '',
    '  assert.deepEqual(badRequestCalls, ["Invalid JSON body (expected object)"]);',
    '  assert.equal(planSessionServiceCalls.length, 0);',
    '  assert.equal(res.statusCode, undefined);',
    '  assert.equal(res.jsonBody, undefined);',
    '});',
    '',
    'test("planSession runtime boundary normalizes undefined body to empty object before service delegation", async () => {',
    '  planSessionServiceCalls.length = 0;',
    '  badRequestCalls.length = 0;',
    '',
    '  const expectedOut = {',
    ...
    '});',
    '',
    'test("planSession runtime boundary normalizes null body to empty object before service delegation", async () => {',
    '  planSessionServiceCalls.length = 0;',
    '  badRequestCalls.length = 0;',
    '',
    '  const expectedOut = {',
