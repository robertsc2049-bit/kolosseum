...
async function runScenario({
  ...
  const acceptedDecision = await httpJson(
    "POST",
    `${baseUrl}/sessions/${sessionId}/events`,
    { event: { type: decisionType } }
  );
  assert.equal(
    acceptedDecision.res.status,
    201,
    `${label}: ${decisionType} expected 201, got ${acceptedDecision.res.status}. raw=${acceptedDecision.text}`
  );
  ...
  try {
    ...
    const acceptedExerciseId = freshBefore.uncachedState.json.current_step?.exercise?.exercise_id ?? null;
    assert.equal(
      acceptedExerciseId,
      baselineExerciseId,
      `${label}: fresh-before live exercise_id drifted before downstream progress`
    );

    const acceptedProgress = await httpJson(
      "POST",
      `${freshServer.baseUrl}/sessions/${sessionId}/events`,
      { event: { type: "COMPLETE_EXERCISE", exercise_id: acceptedExerciseId } }
    );
    assert.equal(
      acceptedProgress.res.status,
      201,
      `${label}: accepted progress after restart expected 201, got ${acceptedProgress.res.status}. raw=${acceptedProgress.text}`
    );
    ...
    const postProgressExerciseId = afterAccepted.uncachedState.json.current_step?.exercise?.exercise_id ?? null;
    assert.ok(
      typeof postProgressExerciseId === "string" && postProgressExerciseId.length > 0,
      `${label}: post-progress live exercise_id missing`
    );
    assert.notEqual(
      postProgressExerciseId,
      acceptedExerciseId,
      `${label}: current_step resurrected stale pre-progress exercise after accepted write`
    );

    assert.equal(
      afterAccepted.uncachedEvents.json.events.length,
      freshBefore.uncachedEvents.json.events.length + 1,
      `${label}: expected exactly one appended runtime event after accepted progress`
    );
    ...
    const replayDecision = await httpJson(
      "POST",
      `${freshServer.baseUrl}/sessions/${sessionId}/events`,
      { event: { type: decisionType } }
    );
    assertRejectedMutation(
      replayDecision,
      `${label} rejected replay of previously accepted return decision after live progress`
    );
    ...
    const afterRejected = await captureLivePair(
      ...
      sessionId,
      freshServer.sessionStateCache,
      `${label} after rejected decision replay`
    );
    assertLivePairStable(afterRejected, { label: `${label} after rejected decision replay` });

    assertByteStableState(
      afterRejected.uncachedState,
      afterAccepted.uncachedState,
      `${label}: rejected decision replay must not drift uncached state`
    );
    assertByteStableEvents(
      afterRejected.uncachedEvents,
      afterAccepted.uncachedEvents,
      `${label}: rejected decision replay must not drift uncached events`
    );
    assertByteStableState(
      afterRejected.cachedState,
      afterAccepted.cachedState,
      `${label}: rejected decision replay must not drift cached state`
    );
    assertByteStableEvents(
      afterRejected.cachedEvents,
      afterAccepted.cachedEvents,
      `${label}: rejected decision replay must not drift cached events`
    );
    assertEventIdentityStable(
      afterRejected.uncachedEvents,
      afterAcceptedSnapshot,
      `${label}: rejected decision replay must preserve event identity/order`
    );
    ...
    const reloadAfterRejected = await captureLivePair(
      ...
      sessionId,
      freshServer.sessionStateCache,
      `${label} reload after rejected decision replay`
    );
    assertLivePairStable(reloadAfterRejected, { label: `${label} reload after rejected decision replay` });
...
test("v0 fresh restart decision replay safety: compile-created live session rejects stale RETURN_CONTINUE replay after accepted live progress", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue fresh restart decision replay rejected after live progress scenario"
    });
  });
});

test("v0 fresh restart decision replay safety: compile-created live session rejects stale RETURN_SKIP replay after accepted live progress", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip fresh restart decision replay rejected after live progress scenario"
    });
  });
});