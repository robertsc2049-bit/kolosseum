...
const verifierPath = path.join(repoRoot, "ci", "scripts", "run_coach_history_counts_demo_verifier.mjs");

function writeTempFile(content) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "p169-demo-"));
  const filePath = path.join(dir, "P169_COACH_HISTORY_COUNTS_DEMO.md");
  fs.writeFileSync(filePath, content.replace(/\r\n/g, "\n"), "utf8");
  return filePath;
}

function runVerifier(filePath) {
  return execFileSync(process.execPath, [verifierPath, filePath], {
    cwd: repoRoot,
    encoding: "utf8",
...
function runVerifierExpectFailure(filePath) {
  try {
    runVerifier(filePath);
...
const goodDoc = `# P169 — Coach History Counts Demo
...
test("P169 verifier passes on counts-only demo", () => {
  const filePath = writeTempFile(goodDoc);
  const stdout = runVerifier(filePath);
  assert.match(stdout, /coach_history_counts_demo_verifier: OK/);
});

test("P169 verifier fails when banned term leaks into allowed copy", () => {
  const badDoc = goodDoc.replace("- Counts by event type", "- Performance trend");
  const filePath = writeTempFile(badDoc);
  const stderr = runVerifierExpectFailure(filePath);
  assert.match(stderr, /banned semantic leaked into allowed copy/i);
});

test("P169 verifier fails when forbidden reference appears in reference fence", () => {
  ...
  const filePath = writeTempFile(badDoc);
  const stderr = runVerifierExpectFailure(filePath);
  assert.match(stderr, /forbidden reference surfaced in reference fence|forbidden surface referenced/i);
});

test("P169 verifier fails when an allowed counter is missing", () => {
  const badDoc = goodDoc.replace("- runtime_event_count", "");
  const filePath = writeTempFile(badDoc);
  const stderr = runVerifierExpectFailure(filePath);
  assert.match(stderr, /allowed counter missing/i);
});