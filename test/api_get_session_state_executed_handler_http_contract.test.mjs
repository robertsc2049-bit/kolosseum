...
test("getSessionState executed path: returns 200 with delegated JSON payload when query succeeds", async () => {
  ...
  const { getSessionState } = await import(`${distHandlerUrl}?case=ok`);
  const req = makeReq({
    params: {
      session_id: "sess_123"
    }
...
test("getSessionState executed path: missing session_id throws 400 badRequest", async () => {
  ...
  const { getSessionState } = await import(`${distHandlerUrl}?case=missing_session_id`);
  const req = makeReq();
  const res = makeRes();
...
test("getSessionState executed path: delegated not-found error preserves explicit error contract", async () => {
  ...
  const { getSessionState } = await import(`${distHandlerUrl}?case=not_found`);
  const req = makeReq({
    params: {
