...
test("CI: registry_law_guard hard-fails on FK break (exercise.equipment token -> not in movement-scoped equipment_tokens)", () => {
  ...
  try {
    ...
    assert.ok(Array.isArray(entry.equipment) && entry.equipment.length > 0);

    entry.equipment[0] = "__fk_break_bad_equipment__";
    ...
    assert.match(combined, /equipment token[\s\S]*not in vocab/i);
    assert.match(combined, /entries\.[a-z0-9_]+:\s*equipment token/i);
  } finally {
