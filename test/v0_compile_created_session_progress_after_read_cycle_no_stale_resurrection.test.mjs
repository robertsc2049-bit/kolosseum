...
async function captureStateThenProgress(baseUrl, sessionId, sessionStateCache, label) {
  sessionStateCache.clear();

  const beforeState = await getState(baseUrl, sessionId, `${label} before state`);
  const beforeEvents = await getEvents(baseUrl, sessionId, `${label} before events`);

  assertLifecycleStateContract(beforeState, {
    label: `${label} before state`,
    expectDecisionRequired: false,
    expectCurrentStep: "EXERCISE"
  });

  const beforeEventSnapshot = snapshotEvents(beforeEvents);
  const beforeExerciseId = beforeState.json.current_step?.exercise?.exercise_id ?? null;
  assert.ok(
    typeof beforeExerciseId === "string" && beforeExerciseId.length > 0,
    `${label}: expected before exercise_id. raw=${JSON.stringify(beforeState.json)}`
  );

  await completeCurrentExercise(baseUrl, sessionId, beforeState, `${label} progress complete`);

  const afterState = await getState(baseUrl, sessionId, `${label} after state`);
  const afterEvents = await getEvents(baseUrl, sessionId, `${label} after events`);
  const afterStateReload = await getState(baseUrl, sessionId, `${label} after state reload`);

  assertLifecycleStateContract(afterState, {
    label: `${label} after state`,
    expectDecisionRequired: false,
    expectCurrentStep: "EXERCISE"
  });
  assertLifecycleStateContract(afterStateReload, {
    label: `${label} after state reload`,
    expectDecisionRequired: false,
    expectCurrentStep: "EXERCISE"
  });

  assertByteStableState(afterStateReload, afterState, `${label}: after state reload parity`);

  const afterExerciseId = afterState.json.current_step?.exercise?.exercise_id ?? null;
  assert.ok(
    typeof afterExerciseId === "string" && afterExerciseId.length > 0,
    `${label}: expected after exercise_id. raw=${JSON.stringify(afterState.json)}`
  );
  assert.notEqual(
    afterExerciseId,
    beforeExerciseId,
    `${label}: current_step exercise_id resurrected stale pre-progress value`
  );

  assert.equal(
    afterEvents.json.events.length > beforeEvents.json.events.length,
    true,
    `${label}: events did not grow after valid downstream progress`
  );

  const afterSnapshot = snapshotEvents(afterEvents);
  assert.equal(
    afterSnapshot.length > beforeEventSnapshot.length,
    true,
    `${label}: event snapshot did not grow after progress`
  );

  for (let i = 0; i < beforeEventSnapshot.length; i += 1) {
    assert.deepEqual(
      afterSnapshot[i],
      beforeEventSnapshot[i],
      `${label}: append-only event prefix drifted at index ${i}`
    );
  }

  return { beforeState, beforeEvents, afterState, afterEvents, afterStateReload };
}
...
async function runScenario({
  ...
  const exerciseProgress = await captureStateThenProgress(
    baseUrl,
    sessionId,
    sessionStateCache,
  ...
  const postDecisionProgress = await captureStateThenProgress(
    baseUrl,
    sessionId,
    sessionStateCache,
    `${label} post decision progress`
  );
  ...
  assertByteStableState(fresh.state, terminalState, `${label}: fresh terminal state parity`);
  assertByteStableEvents(fresh.events, terminalEvents, `${label}: fresh terminal events parity`);

  assert.notEqual(
    exerciseProgress.afterState.json.current_step?.exercise?.exercise_id ?? null,
    exerciseProgress.beforeState.json.current_step?.exercise?.exercise_id ?? null,
    `${label}: first progress seam resurrected stale pre-progress exercise after downstream write`
  );

  assert.notEqual(
    postDecisionProgress.afterState.json.current_step?.exercise?.exercise_id ?? null,
    postDecisionProgress.beforeState.json.current_step?.exercise?.exercise_id ?? null,
    `${label}: post-decision progress seam resurrected stale pre-progress exercise after downstream write`
  );

  const gatedSnapshot = snapshotEvents(gatedEvents);
  const postDecisionBeforeSnapshot = snapshotEvents(postDecisionProgress.beforeEvents);
  assert.equal(
    postDecisionBeforeSnapshot.length > gatedSnapshot.length,
    true,
    `${label}: resolving return decision should append events before post-decision progress probe`
  );

  assertEventIdentityStable(
    fresh.events,
    snapshotEvents(terminalEvents),
    `${label}: fresh terminal event identity`
  );
}
...
test("v0 progress-after-read: compile-created session cannot resurrect stale state after downstream progress (RETURN_CONTINUE)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue progress after read no stale resurrection scenario"
    });
  });
});

test("v0 progress-after-read: compile-created session cannot resurrect stale state after downstream progress (RETURN_SKIP)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip progress after read no stale resurrection scenario"
    });
  });
});