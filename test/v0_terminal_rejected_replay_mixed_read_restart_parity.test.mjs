...
test(
  "test(v0): prove terminal rejected replay preserves mixed-read restart parity",
  async (t) => {
    ...
    const terminalBaselineStateSnapshot = cloneJson(terminalState);
    const terminalBaselineEventsSnapshot = await httpJson(
      "GET",
      `${http.baseUrl}/sessions/${sessionId}/events`
    );
    assertEventsPayload(terminalBaselineEventsSnapshot, "terminal baseline events");
    ...
    const replayCompleteRejected = await httpJson(
      "POST",
      `${http.baseUrl}/sessions/${sessionId}/events`,
      { event: { type: "COMPLETE_EXERCISE", exercise_id: lastCompletedExerciseId } }
    );
    assert.notEqual(
      replayCompleteRejected.res.status,
      201,
      `replayed COMPLETE_EXERCISE must be rejected after terminal progress. raw=${replayCompleteRejected.text}`
    );
    assert.ok(
      [400, 409, 422].includes(replayCompleteRejected.res.status),
      `replayed COMPLETE_EXERCISE expected 400/409/422, got ${replayCompleteRejected.res.status}. raw=${replayCompleteRejected.text}`
    );

    const stateAfterRejectedReplays = await httpJson(
      "GET",
      `${http.baseUrl}/sessions/${sessionId}/state`
    );
    const eventsAfterRejectedReplays = await httpJson(
      "GET",
      `${http.baseUrl}/sessions/${sessionId}/events`
    );

    assertAcceptedSkipTerminalState(
      stateAfterRejectedReplays,
      expectedCompletedIds,
      expectedDroppedIds,
      "state after rejected replays"
    );
    assertEventsPayload(eventsAfterRejectedReplays, "events after rejected replays");

    assertStablePayload(
      stateAfterRejectedReplays,
      terminalBaselineStateSnapshot,
      "state after rejected replays vs terminal baseline"
    );
    assertStablePayload(
      eventsAfterRejectedReplays,
      terminalBaselineEventsSnapshot,
      "events after rejected replays vs terminal baseline"
    );
    ...
    assertStablePayload(warmCycle.state1, terminalBaselineStateSnapshot, "warm cycle state1 vs terminal baseline");
    assertStablePayload(warmCycle.events1, terminalBaselineEventsSnapshot, "warm cycle events1 vs terminal baseline");
    ...
    assertStablePayload(coldCycle.state1, terminalBaselineStateSnapshot, "cold cycle state1 vs terminal baseline");
    assertStablePayload(coldCycle.events1, terminalBaselineEventsSnapshot, "cold cycle events1 vs terminal baseline");
    ...
    assertStablePayload(coldCycle.state1, warmCycle.state1, "cold cycle state1 vs warm cycle state1");
    assertStablePayload(coldCycle.events1, warmCycle.events1, "cold cycle events1 vs warm cycle events1");
  }
);