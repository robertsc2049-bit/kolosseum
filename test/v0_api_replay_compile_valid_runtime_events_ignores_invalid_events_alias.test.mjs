...
test(
  "test(v0): prove /blocks/compile accepts valid runtime_events and ignores invalid events alias when both keys are present",
  async (t) => {
    ...
    const dualWithInvalidAliasNonArray = await replayProjection(
      http.baseUrl,
      phase1,
      {
        phase1_input: phase1,
        runtime_events: continueEvents,
        events: { type: "RETURN_SKIP" },
      },
      "dual-with-invalid-alias-non-array"
    );

    const dualWithInvalidAliasMixed = await replayProjection(
      http.baseUrl,
      phase1,
      {
        phase1_input: phase1,
      ...
      "dual-with-invalid-alias-mixed"
    );

    assert.deepEqual(
      dualWithInvalidAliasNonArray.projection,
      continueOnly.projection,
      `invalid alias non-array must not poison valid runtime_events projection.\ndual=${JSON.stringify(dualWithInvalidAliasNonArray.projection)}\nbase=${JSON.stringify(continueOnly.projection)}`
    );

    assert.deepEqual(
      dualWithInvalidAliasMixed.projection,
      continueOnly.projection,
      `invalid alias mixed array must not poison valid runtime_events projection.\ndual=${JSON.stringify(dualWithInvalidAliasMixed.projection)}\nbase=${JSON.stringify(continueOnly.projection)}`
    );

    assert.deepEqual(
      dualWithInvalidAliasNonArray.eventTypes,
      continueOnly.eventTypes,
      `invalid alias non-array must not change surfaced runtime event sequence.\ndual=${JSON.stringify(dualWithInvalidAliasNonArray.eventTypes)}\nbase=${JSON.stringify(continueOnly.eventTypes)}`
    );

    assert.deepEqual(
      dualWithInvalidAliasMixed.eventTypes,
      continueOnly.eventTypes,
      `invalid alias mixed array must not change surfaced runtime event sequence.\ndual=${JSON.stringify(dualWithInvalidAliasMixed.eventTypes)}\nbase=${JSON.stringify(continueOnly.eventTypes)}`
    );

    assert.equal(
      dualWithInvalidAliasNonArray.projection.execution_status,
      "completed",
      `invalid alias non-array dual payload should still classify from runtime_events path.\nprojection=${JSON.stringify(dualWithInvalidAliasNonArray.projection)}`
    );

    assert.equal(
      dualWithInvalidAliasMixed.projection.execution_status,
      "completed",
      `invalid alias mixed dual payload should still classify from runtime_events path.\nprojection=${JSON.stringify(dualWithInvalidAliasMixed.projection)}`
    );

    assert.equal(
      dualWithInvalidAliasNonArray.projection.trace.dropped_ids.length,
      0,
      `invalid alias non-array dual payload should preserve no-drop truth.\nprojection=${JSON.stringify(dualWithInvalidAliasNonArray.projection)}`
    );

    assert.equal(
      dualWithInvalidAliasMixed.projection.trace.dropped_ids.length,
      0,
      `invalid alias mixed dual payload should preserve no-drop truth.\nprojection=${JSON.stringify(dualWithInvalidAliasMixed.projection)}`
    );
  }
);