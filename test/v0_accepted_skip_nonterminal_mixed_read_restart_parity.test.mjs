...
function assertSkipPathNonterminalState(
  statePayload,
  ...
  assert.ok(
    ...
    `${label}: expected nonterminal remaining_ids after accepted skip.\ntrace=${JSON.stringify(trace)}`
  );
...
test(
  "test(v0): prove accepted skip path preserves mixed-read restart parity at nonterminal downstream state",
  async (t) => {
    ...
    assert.ok(
      ...
      `expected at least two remaining ids so skip path can stay nonterminal.\ntrace=${JSON.stringify(splitState.json?.trace)}`
    );
    assert.equal(
      splitRemainingIds[0],
      secondExerciseId,
    ...
    const expectedCompletedIds = [firstExerciseId];
    const expectedDroppedIds = [secondExerciseId];

    assertSkipPathNonterminalState(
      stateAfterSkip,
      expectedCompletedIds,
      expectedDroppedIds,
      "state after accepted skip"
    );
    ...
    assert.notEqual(
      ...
      201,
      `replayed RETURN_CONTINUE must be rejected after accepted skip. raw=${replayContinueRejected.text}`
    );
    assert.ok(
    ...
    assert.deepEqual(
      ...
      `unexpected event order after accepted skip nonterminal state.\ngot ${JSON.stringify(warmCycle.events1.json.events)}`
    );
