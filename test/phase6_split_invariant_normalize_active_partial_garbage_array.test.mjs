...
// normalize MUST coerce it to [] and MUST NOT throw.
{
  ...
  const cases = [
    {
      name: "remaining_at_split_ids has non-string elements",
      last_seq: 9001,
      runtime: {
        ...
        skipped_ids: [],
        split_active: true,
        remaining_at_split_ids: [123, null]
      }
    },
    {
  ...
  for (const c of cases) {
    ...
    assert.equal(needsUpgrade, true, "normalize should upgrade/repair invalid remaining_at_split_ids: [123, null]split" in summary.runtime, c.name + " (split must be emitted when active)");
