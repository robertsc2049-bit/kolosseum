...
function assertSkipPathStateAfterTwoDownstreamSteps(
  statePayload,
...
test(
  "test(v0): prove accepted skip path preserves mixed-read restart parity after two downstream completions",
  async (t) => {
    ...
    assert.notEqual(
      ...
      201,
      `replayed RETURN_SKIP must be rejected after accepted skip + two downstream completions. raw=${replaySkipRejected.text}`
    );
    assert.ok(
    ...
    assert.deepEqual(
      ...
      `unexpected event order after accepted skip + two downstream completions.\ngot ${JSON.stringify(warmCycle.events1.json.events)}`
    );
