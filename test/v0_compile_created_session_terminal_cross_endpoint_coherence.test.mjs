...
async function listBlockSessions(baseUrl, blockId, label) {
  ...
  const sessions = payload.json.sessions ?? payload.json.block_sessions ?? payload.json.items ?? null;
  assert.ok(
    Array.isArray(sessions),
    `${label}: expected sessions array on block sessions payload. raw=${payload.text}`
  );
...
function normalizeBlockSessionRow(row) {
  return {
    session_id: row?.session_id ?? null,
    block_id: row?.block_id ?? null,
    status: row?.status ?? row?.session_status ?? null,
    current_step_type: row?.current_step_type ?? row?.current_step?.type ?? null,
    has_current_step: row?.current_step != null,
    return_decision_required:
      row?.trace?.return_decision_required ??
      row?.return_decision_required ??
      null
  };
}

function findBlockSessionRow(payload, sessionId, label) {
  const row = payload.sessions.find((entry) => (entry?.session_id ?? null) === sessionId) ?? null;
  assert.ok(
    row,
    `${label}: expected to find session_id=${sessionId} in block sessions payload. raw=${payload.text}`
  );
  return row;
}

function assertTerminalBlockSessionRow(row, { label, sessionId, blockId }) {
  const normalized = normalizeBlockSessionRow(row);

  assert.equal(
    normalized.session_id,
    sessionId,
    `${label}: block session row session_id mismatch. row=${JSON.stringify(row)}`
  );
  assert.equal(
    normalized.block_id,
    blockId,
    `${label}: block session row block_id mismatch. row=${JSON.stringify(row)}`
  );

  if (normalized.status !== null) {
    assert.match(
      String(normalized.status),
      /complete|completed|terminal/i,
      `${label}: expected terminal-like status in block session row. row=${JSON.stringify(row)}`
    );
  }

  if (normalized.current_step_type !== null) {
    assert.equal(
      normalized.current_step_type,
      null,
      `${label}: expected no current step type at terminal row. row=${JSON.stringify(row)}`
    );
  }

  if (normalized.has_current_step) {
    assert.equal(
      normalized.has_current_step,
      false,
      `${label}: expected no current_step payload at terminal row. row=${JSON.stringify(row)}`
    );
  }

  if (normalized.return_decision_required !== null) {
    assert.equal(
      normalized.return_decision_required,
      false,
      `${label}: expected return_decision_required=false at terminal row. row=${JSON.stringify(row)}`
    );
  }
}
...
async function readTerminalTriplet(baseUrl, { sessionId, blockId, sessionStateCache, label }) {
  sessionStateCache.clear();

  const state = await getState(baseUrl, sessionId, `${label} state`);
  const events = await getEvents(baseUrl, sessionId, `${label} events`);
  const blockSessions = await listBlockSessions(baseUrl, blockId, `${label} block sessions`);
  ...
  const row = findBlockSessionRow(blockSessions, sessionId, `${label} block sessions`);
  assertTerminalBlockSessionRow(row, { label: `${label} block session row`, sessionId, blockId });

  return { state, events, blockSessions, row };
}

async function readViaFreshServer({ root, databaseUrl, sessionId, blockId, label }) {
  const buildEnv = {
    ...process.env,
  ...
  try {
    ...
    return await readTerminalTriplet(baseUrl, {
      sessionId,
      blockId,
      sessionStateCache,
      label: `${label} fresh`
...
async function runScenario({
  ...
  await advanceToTerminal(baseUrl, sessionId, sessionStateCache, `${label} terminalize`);

  const baseline = await readTerminalTriplet(baseUrl, {
    sessionId,
    blockId,
    sessionStateCache,
  ...
  const reload = await readTerminalTriplet(baseUrl, {
    sessionId,
    blockId,
    sessionStateCache,
    label: `${label} reload`
  ...
  assertByteStableState(reload.state, baseline.state, `${label}: reload state parity`);
  assertByteStableEvents(reload.events, baseline.events, `${label}: reload events parity`);
  assert.deepEqual(
    normalizeBlockSessionRow(reload.row),
    normalizeBlockSessionRow(baseline.row),
    `${label}: reload block session row parity drifted`
  );

  const baselineSnapshot = snapshotEvents(baseline.events);
  assertEventIdentityStable(reload.events, baselineSnapshot, `${label}: reload event identity parity`);
  ...
  assertByteStableState(fresh.state, baseline.state, `${label}: fresh state parity`);
  assertByteStableEvents(fresh.events, baseline.events, `${label}: fresh events parity`);
  assert.deepEqual(
    normalizeBlockSessionRow(fresh.row),
    normalizeBlockSessionRow(baseline.row),
    `${label}: fresh block session row parity drifted`
  );
  assertEventIdentityStable(fresh.events, baselineSnapshot, `${label}: fresh event identity parity`);

  const terminalStateJson = baseline.state.json;
  assert.equal(
    terminalStateJson.current_step ?? null,
    null,
    `${label}: terminal /state should expose null current_step`
  );

  const baselineRowNormalized = normalizeBlockSessionRow(baseline.row);
  assert.equal(
    baselineRowNormalized.session_id,
    sessionId,
    `${label}: block session row lost session identity`
  );
  assert.equal(
    baselineRowNormalized.block_id,
    blockId,
    `${label}: block session row lost block identity`
  );

  assert.equal(
    baseline.events.json.events.length > 0,
    true,
    `${label}: terminal events should remain readable and non-empty`
  );
}
...
test("v0 terminal coherence: compile-created session stays cross-endpoint coherent after terminalization (RETURN_CONTINUE)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue terminal cross-endpoint coherence scenario"
    });
  });
});

test("v0 terminal coherence: compile-created session stays cross-endpoint coherent after terminalization (RETURN_SKIP)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip terminal cross-endpoint coherence scenario"
    });
  });
});