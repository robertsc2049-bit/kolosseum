...
test("decision summary consumer surface manifest remains present in composed test:ci command set", () => {
  const repo = process.cwd();
  const manifestPath = path.join(repo, "ci", "contracts", "decision_summary_consumer_surface_ci_cluster.json");
  ...
  assert.ok(manifest.cluster.length > 0, "expected non-empty decision summary consumer surface cluster manifest");
