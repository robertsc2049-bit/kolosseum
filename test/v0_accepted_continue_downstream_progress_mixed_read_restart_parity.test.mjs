...
test(
  "test(v0): prove accepted continue path preserves mixed-read restart parity after downstream progress",
  async (t) => {
    ...
    assertContinuePathState(
      stateAfterComplete2,
      expectedCompletedIds,
      "state after downstream progress"
    );
    ...
    if (thirdExerciseId) {
      assert.notEqual(
        thirdExerciseId,
        secondExerciseId,
        `expected downstream progress to advance beyond second exercise.\nstate=${stateAfterComplete2.text}`
      );
    }
    ...
    assert.deepEqual(
      ...
      `unexpected event order after accepted continue path progress.\ngot ${JSON.stringify(warmCycle.events1.json.events)}`
    );
