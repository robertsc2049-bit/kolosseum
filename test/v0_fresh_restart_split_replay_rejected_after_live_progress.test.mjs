...
async function runScenario({
  ...
  try {
    ...
    const replaySplit = await httpJson(
      "POST",
      `${freshServer.baseUrl}/sessions/${sessionId}/events`,
      { event: { type: "SPLIT_SESSION" } }
    );
    assertRejectedMutation(
      replaySplit,
      `${label} rejected replay of stale SPLIT_SESSION after live progress`
    );
    ...
    const afterRejected = await captureLivePair(
      ...
      sessionId,
      freshServer.sessionStateCache,
      `${label} after rejected split replay`
    );
    assertLivePairStable(afterRejected, { label: `${label} after rejected split replay` });

    assertByteStableState(
      afterRejected.uncachedState,
      afterAccepted.uncachedState,
      `${label}: rejected split replay must not drift uncached state`
    );
    assertByteStableEvents(
      afterRejected.uncachedEvents,
      afterAccepted.uncachedEvents,
      `${label}: rejected split replay must not drift uncached events`
    );
    assertByteStableState(
      afterRejected.cachedState,
      afterAccepted.cachedState,
      `${label}: rejected split replay must not drift cached state`
    );
    assertByteStableEvents(
      afterRejected.cachedEvents,
      afterAccepted.cachedEvents,
      `${label}: rejected split replay must not drift cached events`
    );
    assertEventIdentityStable(
      afterRejected.uncachedEvents,
      afterAcceptedSnapshot,
      `${label}: rejected split replay must preserve event identity/order`
    );
    ...
    const reloadAfterRejected = await captureLivePair(
      ...
      sessionId,
      freshServer.sessionStateCache,
      `${label} reload after rejected split replay`
    );
    assertLivePairStable(reloadAfterRejected, { label: `${label} reload after rejected split replay` });
...
test("v0 fresh restart split replay safety: compile-created live session rejects stale SPLIT_SESSION replay after accepted live progress (RETURN_CONTINUE)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      ...
      label: "return-continue fresh restart split replay rejected after live progress scenario"
    });
  });
});

test("v0 fresh restart split replay safety: compile-created live session rejects stale SPLIT_SESSION replay after accepted live progress (RETURN_SKIP)", async (t) => {
  await withServer(t, async ({ baseUrl, root, databaseUrl, sessionStateCache }) => {
    await runScenario({
      baseUrl,
      root,
      ...
      label: "return-skip fresh restart split replay rejected after live progress scenario"
    });
  });
});