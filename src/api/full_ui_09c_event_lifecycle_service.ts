...
export async function unlinkAthleteFromStandaloneEvent(
  inputValue: unknown
): Promise<Readonly<JsonRecord>> {
  if (!isRecord(inputValue)) {
  ...
  exactKeys(
    inputValue,
    ["coach_user_id", "event_id", "athlete_user_id"],
    "event_unlink"
  );
  ...
  await requireActiveCoach(coachUserId);
  await assertRelationship(coachUserId, athleteUserId);
  const client = await pool.connect();
  ...
  try {
    ...
    await requireOwnedEvent(client, coachUserId, eventId, true);
    const currentLink = await latestLinkByPair(
      client,
      coachUserId,
      athleteUserId,
    ...
    if (!currentLink || currentLink.link_state !== "linked") {
    ...
    const withoutHash = deepFreeze({
      ...
      link_state: "unlinked",
      link_revision: linkRevision(currentLink) + 1,
      previous_link_record_sha256: cleanString(currentLink.record_sha256),
      lifecycle_action: "athlete_unlinked",
      updated_at_iso8601: occurredAt,
      unlinked_at_iso8601: occurredAt,
      engine_visible: false
    });
    delete (withoutHash as JsonRecord).record_sha256;
