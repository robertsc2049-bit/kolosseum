// DEV NOTE: FULL-UI-95 org-owner support/error-reporting parity. Mirrors
// product_support_service.ts (FULL-UI-20) exactly, targeting
// org_owner_support_requests instead of product_support_requests - a
// support report captures only an explicit, narrow allowlist of caller-
// supplied context fields, never a raw error payload or any other
// unbounded caller-supplied blob.
...
export class OrgOwnerSupportError extends Error {
  readonly status: number;

  constructor(message: string, status = 400) {
    super(message);
    this.name = "OrgOwnerSupportError";
    this.status = status;
  }
}
...
export async function createOrgOwnerSupportReport(
  userId: string,
  inputValue: unknown
): Promise<Readonly<JsonRecord>> {
  if (!userId) {
    throw new OrgOwnerSupportError("org_owner_support_report_user_required", 401);
  }
  if (!isRecord(inputValue)) {
    throw new OrgOwnerSupportError("org_owner_support_report_input_invalid");
  }

  for (const key of Object.keys(inputValue)) {
    if (!supportReportInputKeys.has(key)) {
      throw new OrgOwnerSupportError("org_owner_support_report_unknown_field");
    }
  }

  const correlationId = cleanString(inputValue.correlation_id);
  if (!/^[a-zA-Z0-9-]{8,64}$/u.test(correlationId)) {
    throw new OrgOwnerSupportError("org_owner_support_report_correlation_id_invalid");
  }

  if (!isIso8601(inputValue.occurred_at_iso8601)) {
    throw new OrgOwnerSupportError("org_owner_support_report_timestamp_invalid");
  }

  const description = cleanString(inputValue.description);
  if (!description || description.length > 4000) {
    throw new OrgOwnerSupportError("org_owner_support_report_description_invalid");
  }
  ...
  const existing = await pool.query(
    `SELECT * FROM org_owner_support_requests WHERE correlation_id = $1`,
    [correlationId]
  );

  if (existing.rows[0]) {
    if (cleanString(existing.rows[0].user_id) !== userId) {
      throw new OrgOwnerSupportError("org_owner_support_report_correlation_id_conflict", 409);
    }
  ...
  const inserted = await pool.query(
    `
    INSERT INTO org_owner_support_requests (
      correlation_id,
      user_id,
      route_hash,
      occurred_at,
  ...
  if (!row) {
    throw new OrgOwnerSupportError("org_owner_support_report_persist_readback_failed", 500);
  }

  return toPublicReport(row);
}

export async function listOrgOwnerSupportReportsForUser(
  userId: string
